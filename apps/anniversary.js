...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .ann-soft-layer{position:absolute;inset:0;z-index:0;pointer-events:none;background:color-mix(in srgb,var(--bg-primary) 78%,transparent)}
    .ann-nav{position:fixed;top:0;left:0;right:0;z-index:100;height:calc(56px + env(safe-area-inset-top));display:flex;align-items:center;gap:var(--spacing-sm);padding:env(safe-area-inset-top) 20px 0;background:var(--surface-glass);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
    ...
    .ann-hero{padding:22px;border-radius:28px;background:var(--bg-card);box-shadow:var(--shadow-md)}
    ...
    .ann-hero-text{margin-top:8px;color:var(--text-secondary);font-size:var(--font-size-small);line-height:1.6}
    ...
    .ann-calendar{margin-top:var(--spacing-md);padding:var(--spacing-md);border-radius:28px;background:var(--bg-card);box-shadow:var(--shadow-sm)}
    ...
    .ann-card{padding:var(--spacing-md);border-radius:var(--radius-lg);background:var(--bg-card);box-shadow:var(--shadow-sm);transition:var(--motion);cursor:pointer}
    ...
    .ann-action-btn:active{transform:scale(.96)}
    .ann-action-btn.danger{color:var(--accent-dark)}
    .ann-empty{min-height:220px;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:var(--spacing-sm);padding:var(--spacing-lg);border-radius:24px;background:var(--bg-card);box-shadow:var(--shadow-sm);color:var(--text-secondary);text-align:center}
    ...
    .ann-input,.ann-textarea{width:100%;border-radius:var(--radius-md);background:var(--surface-muted);color:var(--text-primary);font-size:var(--font-size-base)}
    .ann-input{min-height:46px;padding:10px var(--spacing-md)}
    .ann-textarea{min-height:120px;padding:12px var(--spacing-md);line-height:1.6}
    ...
    .ann-marker-btn:active,.ann-mini-btn:active,.ann-manage-row:active{transform:scale(.96)}
    ...
    .ann-custom-section{padding:var(--spacing-md);border-radius:var(--radius-lg);background:var(--bg-card);box-shadow:var(--shadow-sm);margin-bottom:var(--spacing-md)}
...
function sanitizeVisualMetadata() {
  ...
  Object.entries(visuals).forEach(([id, value]) => {
    ...
    const keys = ['image', 'iconImage', 'backgroundImage', 'imageBase64', 'imageSource', 'value', 'data', 'src', 'url', 'base64'];
    if (keys.some((key) => key in value)) changed = true;
  });
...
async function loadVisualCache(list) {
  ...
  await Promise.all(ids.map(async (id) => {
    ...
    if (record?.value) visualCache.set(id, record.value);
  }));
}
...
function openCustomizeSheet() {
  ...
  const itemSection = createCustomSection('小图管理', '每个纪念日都能换成自己的小图片。');
...
function createManageRow(item) {
  ...
  row.append(thumb, main, imageButton, clearButton);
  return row;
}
