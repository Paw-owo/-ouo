...
function renderList() {
  ...
  titleWrap.append(
    el('div', 'nav-title', currentTab === 'ai' ? '角色' : '我的人设'),
    el('div', 'nav-subtitle', currentTab === 'ai'
      ? (characters.length ? `${characters.length} 个角色` : '创建你的第一个 AI 角色')
      : (userProfiles.length ? `${userProfiles.length} 份人设` : '创建你的专属人设')
    )
  );
...
function renderProfileEmptyState() {
  ...
  box.append(
    mark,
    el('div', 'character-empty-title', '还没有你的专属人设'),
    el('div', 'character-empty-text', '创建一份“我的小档案”，写下你怎么称呼自己、喜欢什么、想让 AI 怎么理解你。')
  );
...
function createProfileCard(profile) {
  ...
  if (profile.avatar) {
    ...
    avatar.appendChild(createIcon('star', 30));
  }
  ...
  info.append(
    ...
    el('div', 'profile-meta', profile.isDefault ? '默认人设' : '可绑定到指定角色')
  );
  ...
  card.addEventListener('click', () => openProfileEditor(profile.id));
...
function getUserProfileNameById(profileId) {
  ...
  return profile?.name || '已绑定用户人设';
}

function getProfileDescPreview(profile) {
  const text = String(profile.content || '').trim();
  if (!text) return '还没填写内容';
...
function openProfileActions(profile) {
  ...
  const sheet = sheetBase(profile.name || '人设操作', '可设为默认，也可以导出备份或删除。');

  const editButton = button('编辑人设', 'ghost', 'edit');
  editButton.addEventListener('click', () => {
    hideBottomSheet();
    window.setTimeout(() => openProfileEditor(profile.id), 180);
  });

  const defaultButton = button(profile.isDefault ? '取消默认人设' : '设为默认人设', 'ghost', profile.isDefault ? 'check' : 'star');
  defaultButton.addEventListener('click', async () => {
    await toggleDefaultProfile(profile.id);
    hideBottomSheet();
  });

  const exportButton = button('导出人设', 'ghost', 'download');
  exportButton.addEventListener('click', async () => {
    await exportProfile(profile.id);
    hideBottomSheet();
  });

  const deleteButton = button('删除人设', 'ghost', 'delete');
  deleteButton.addEventListener('click', async () => {
    hideBottomSheet();
...
function createProfileSelect(value, onChange) {
  ...
  const options = [
    { value: '', label: '跟随默认人设' },
    { value: 'none', label: '不绑定人设' }
  ];

  userProfiles.forEach((profile) => {
    options.push({ value: profile.id, label: profile.name || '未命名人设' });
  });
...
async function openProfileEditor(profileId = '') {
  ...
  const title = el('div', 'sheet-title', existing ? '编辑我的人设' : '新建我的人设');
  const desc = el('div', 'sheet-description', '这是你的小档案。AI 会按这份资料理解你，也可以绑定到指定角色。');
  ...
  const contentInput = textarea('写一下你的性格、爱好、说话习惯、想让 AI 怎么理解你', draft.content);
  ...
  core.append(
    ...
    field('人设内容', contentInput),
    customRow('设为默认人设', defaultSwitch)
  );

  const note = createSoftNote('设为默认后，没单独绑定档案的 AI 角色都会优先使用这份资料。');
  ...
  actions.append(cancelButton, saveButton);
  sheet.append(title, desc, core, note, actions);
...
function renderBackgroundEditor(draft) {
  ...
  const mode = createSegmented(
    ...
    (value) => {
      ...
      if (value === 'color' && !draft.chatBackground.value) draft.chatBackground.value = getDefaultColorValue();
...
function renderImportExportTools(draft, isExisting) {
  ...
  box.append(
    createSoftNote('导出后的 JSON 可以通过右上角导入按钮重新导入。'),
    exportButton
  );
...
async function openProfileImportSheet() {
  const sheet = sheetBase('导入我的人设', '支持本项目导出的我的人设 JSON。');
...
async function saveCharacter(character) {
  const normalized = normalizeCharacter({
    ...
    createdAt: character.createdAt || getNow()
  });
...
async function saveUserProfile(profile) {
  ...
  const next = normalizeProfile({
    ...
    createdAt: profile.createdAt || getNow(),
    updatedAt: getNow()
  });
...
async function deleteProfile(profileId) {
  ...
  const ok = await showConfirm(`确定删除「${target.name || '这个人设'}」吗？角色如果绑定了它，会自动回落到默认人设。`);
  if (!ok) return;
...
async function toggleDefaultProfile(profileId) {
  ...
  await loadData();
  renderList();
  showToast(target.isDefault ? '已设为默认人设' : '已取消默认人设');
}
...
function loadUserProfiles() {
  const raw = getData('app_user_profiles');
  return Array.isArray(raw) ? raw.map(normalizeProfile) : [];
}

function saveUserProfiles(list) {
  setData('app_user_profiles', Array.isArray(list) ? list.map(normalizeProfile) : []);
}
...
// 依赖：../core/storage.js 的 getData/setData/generateId/getNow/getAllDB/getDB/setDB/deleteDB/getByIndexDB/compressImage；../core/ui.js 的 showToast/showBottomSheet/hideBottomSheet/showConfirm/createIcon
