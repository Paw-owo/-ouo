...
function appendAssistantCardLayers(card, message) {
  ...
  if (contentText || message.type === 'image' || message.type === 'sticker' || message.type === 'transfer') {
    appendAssistantContentBlocks(card, contentText, message);
  }
}

function appendAssistantContentBlocks(card, contentText, message = null) {
  const bubbleMode = getSettings().bubbleMode !== 'dialog';

  if (!bubbleMode || message?.type === 'image' || message?.type === 'sticker' || message?.type === 'transfer') {
    card.appendChild(createBubbleBlock(contentText, message));
    return;
  }

  const chunks = splitAssistantBubbleText(contentText);
  if (!chunks.length) {
    card.appendChild(createBubbleBlock(contentText, message));
    return;
  }

  const group = el('div', 'assistant-bubble-stack');
  chunks.forEach((chunk) => group.appendChild(createBubbleBlock(chunk, null)));
...
function splitAssistantBubbleText(text) {
  ...
  chunks.forEach((chunk) => {
    ...
    while (rest.length > 24) {
      let cutIndex = -1;
      const searchArea = rest.slice(10, 22);
      const softMarks = ['，', '、', ',', ' '];

      for (const mark of softMarks) {
        const found = searchArea.lastIndexOf(mark);
        if (found >= 0) {
          cutIndex = 10 + found + 1;
          break;
        }
      ...
      if (cutIndex < 10) cutIndex = 18;

      finalChunks.push(rest.slice(0, cutIndex).trim());
      rest = rest.slice(cutIndex).trim();
    }

    if (rest) finalChunks.push(rest);
  });
