...
function renderList() {
  ...
  const searchInput = input('找一找说过的话');
...
function renderChatScreen() {
  ...
  const dialogMode = settings.bubbleMode === 'dialog';
  ...
  screen.classList.toggle('dialog-mode', dialogMode);
  screen.classList.toggle('bubble-mode', !dialogMode);
...
function createInputBar() {
  ...
  const plusButton = iconButton('add', '展开小工具');
  ...
  textarea.placeholder = currentGroup ? '给大家发句话' : '想和 TA 说什么';
  textarea.rows = 1;
...
function createQuotePreview() {
  ...
  const text = el('div', 'quote-preview-text', `${speaker}：${getMessagePreview(quotedMessage)}`);
...
async function generateAssistantReply(extraInstruction = '') {
  ...
  if (config.streamEnabled !== false) {
    await streamMessage({
      ...
      onError: () => {
        assistantMessage.content = '刚才有点卡住，再试一次好吗？';
      }
    });
  } else {
...
async function generateGroupAssistantReply(character) {
  ...
  const tokenInput = estimateTokens(inputText);
...
function resolveUserProfileForCharacter(character) {
  ...
  return profiles.find((profile) => profile.isDefault === true) || null;
}
...
function createMessageRow(message, isGroup = false) {
  ...
  const dialogMode = getSettings().bubbleMode === 'dialog';
  ...
  if (!isUser || dialogMode) {
    body.appendChild(el('div', 'message-name', isUser ? (getSettings().user.name || '我') : getSpeakerName(message.characterId)));
  }
  ...
  if (Array.isArray(message.toolCalls) && message.toolCalls.length) {
    message.toolCalls.forEach((toolCall) => {
      bubble.appendChild(createToolCallCard(toolCall));
    });
  }

  if (message.type === 'tool' && !message.content) {
    bubble.classList.add('tool-only-bubble');
  } else if (message.type === 'image' && message.imageBase64) {
...
function createThinkingBlock(text) {
  ...
  details.className = 'thinking-card claude-fold-card';

  const summary = document.createElement('summary');
  summary.className = 'fold-summary';

  const icon = el('span', 'fold-icon');
  icon.appendChild(createIcon('memory', 16));

  const main = el('span', 'fold-main');
  main.append(
    el('span', 'fold-title', '思考'),
    el('span', 'fold-text', summarizeThinking(text))
  );

  const arrow = el('span', 'fold-arrow');
  arrow.appendChild(createIcon('arrow-down', 16));

  const content = el('div', 'fold-content thinking-content', text || 'TA 正在整理思路');

  summary.append(icon, main, arrow);
...
function summarizeThinking(text) {
  ...
  return `TA 在想：${clean.slice(0, 28)}`;
}

function createToolCallCard(toolCall) {
  const details = document.createElement('details');
  details.className = `tool-call-card claude-fold-card tool-status-${toolCall.status || 'done'}`;

  const summary = document.createElement('summary');
  summary.className = 'fold-summary';

  const icon = el('span', 'fold-icon');
  icon.appendChild(createIcon(toolCall.status === 'running' ? 'mcp' : toolCall.status === 'error' ? 'clear' : 'check', 16));

  const main = el('span', 'fold-main');
  main.append(
    el('span', 'fold-title', createToolCallSummary(toolCall)),
    el('span', 'fold-text', toolCall.serverName || 'MCP')
  );

  const arrow = el('span', 'fold-arrow');
  arrow.appendChild(createIcon('arrow-down', 16));

  const content = el('div', 'fold-content tool-call-content');
  content.append(
    createToolMetaLine('工具', toolCall.toolName || ''),
    createToolMetaLine('参数', stringifyCompact(toolCall.arguments)),
...
function createToolCallSummary(toolCall) {
  if (toolCall.status === 'running') return `正在使用工具：${toolCall.toolName || '工具'}`;
  if (toolCall.status === 'error') return `工具有点卡住：${toolCall.toolName || '工具'}`;
  return `工具用好啦：${toolCall.toolName || '工具'}`;
}

function createToolMetaLine(label, value) {
  const wrap = el('div', 'tool-meta-line');
  wrap.append(
    el('div', 'tool-meta-label', label),
    el('pre', 'tool-meta-value', value || '暂无')
  );
  return wrap;
}
...
function openToolSheet() {
  ...
  sheet.append(
    ...
    createToolGrid([
      { icon: 'mic', label: '文字语音', action: () => showToast('这里先用文字说话，iOS 更稳') },
      ...
      { icon: 'memory', label: 'TA的小记忆', action: openMemoryPanel },
      { icon: 'settings', label: '聊天小开关', action: openConfigSheet },
      { icon: 'clear', label: '清掉临时小纸条', action: clearContext },
...
function openConfigSheet() {
  ...
  const apiPanel = detailsBlock('回复设置');
  ...
  proactivePanel.append(
    createSoftNote('网页关掉时不能后台运行。下次打开或停留聊天页时，会检查要不要主动说话。'),
    customRow('模式一：等你回复', switchButton(Boolean(config.proactiveMode1Enabled), (active) => {
    ...
    customRow('模式二：在线陪聊', switchButton(Boolean(config.proactiveMode2Enabled), (active) => {
...
function openChatMoreSheet() {
  ...
  sheet.append(
    ...
    createToolGrid([
      { icon: 'settings', label: '聊天小开关', action: openConfigSheet },
      { icon: 'clear', label: '清掉这段聊天', action: clearAllMessages },
...
function openStickerPanel() {
  ...
  function render(filter = '') {
    ...
    if (!grid.children.length) {
      grid.appendChild(createSoftNote('没找到这个表情。'));
    }
  }
...
function openMcpCallSheet(server, tool) {
  ...
  run.addEventListener('click', async () => {
    ...
    if (currentGroup) {
      await setDB('group_messages', toolMessage.id, toolMessage);
    } else if (currentCharacter) {
      await setDB('messages', toolMessage.id, toolMessage);
    }
...
async function openMemoryPanel() {
  ...
  const sheet = el('div', 'memory-sheet');
  sheet.append(
    el('div', 'sheet-title', 'TA的小记忆'),
    el('div', 'sheet-description', '这里能看到 TA 自动记住的小事，也能手动添一条。')
  );

  const editor = el('details', 'memory-editor-inline');
  const summary = document.createElement('summary');
  summary.textContent = '添一条小记忆';

  const content = textarea('比如：我喜欢雨天喝热奶茶', '');
  const save = button('记住这条', 'primary', 'check');
  save.addEventListener('click', async () => {
    const ok = await saveMemoryDraft(null, content.value, 'manual');
    if (!ok) return;
    content.value = '';
    editor.open = false;
    await refreshMemorySheetList();
  });

  editor.append(summary, field('内容', content), save);

  const listEl = el('div', 'memory-manager-list');
  sheet.append(editor, listEl);

  memorySheetState = { sheet, listEl };
  await renderMemoryManager(listEl);

  showBottomSheet(sheet);
}

async function refreshMemorySheetList() {
  if (!memorySheetState?.listEl) return;
  await renderMemoryManager(memorySheetState.listEl);
}
...
async function renderMemoryManager(listEl) {
  ...
  memories.forEach((memory) => {
    listEl.appendChild(createMemoryCard(memory));
  });
}
...
function createMemoryCard(memory) {
  ...
  main.append(
    ...
    el('div', 'memory-card-meta', `${getMemorySourceLabel(memory.source)} · ${formatMemoryTime(memory.createdAt)}`)
  );

  const actions = el('div', 'memory-card-actions');
  const edit = iconButton('edit', '编辑记忆');
  edit.addEventListener('click', () => openMemoryEditSheet(memory));

  const del = iconButton('delete', '删除记忆');
  del.addEventListener('click', () => deleteMemoryItem(memory));
...
function openMemoryEditSheet(memory) {
  if (!memorySheetState?.sheet) return;

  const oldEditor = memorySheetState.sheet.querySelector('.memory-floating-editor');
  oldEditor?.remove();

  const editor = el('div', 'memory-floating-editor');
  editor.append(
    el('div', 'memory-floating-title', '修一下这条小记忆')
  );

  const content = textarea('编辑这条记忆', memory?.content || '');
  const actions = el('div', 'settings-actions');
  const cancel = button('先不改', 'ghost', 'close');
  const save = button('保存这版', 'primary', 'check');

  cancel.addEventListener('click', () => editor.remove());
  save.addEventListener('click', async () => {
    const ok = await saveMemoryDraft(memory, content.value, memory?.source || 'manual');
    if (!ok) return;
    editor.remove();
    await refreshMemorySheetList();
  });
  ...
  editor.append(field('内容', content), actions);

  memorySheetState.sheet.insertBefore(editor, memorySheetState.listEl);
}

async function saveMemoryDraft(memory, content, source = 'manual') {
  ...
  const next = {
    ...
    content: clean,
    source: memory?.source || source || 'manual',
...
function getMemorySourceLabel(value) {
  if (value === 'auto') return '自动记住';
  if (value === 'summary') return '小总结';
  return '手动写下';
}
  ...
  save.addEventListener('click', async () => {
    ...
    if (currentGroup) {
      ...
      for (const item of after) {
        await deleteDB('group_messages', item.id);
      }
...
async function handleSearch(query, resultsBox) {
  ...
  if (!matched.length) {
    resultsBox.appendChild(createSoftNote('没找到相关聊天记录。'));
    return;
  }
...
function getMessagePreview(message, full = false) {
  ...
  if (message.type === 'tool') {
    ...
    return toolCall ? createToolCallSummary(toolCall) : '[工具]';
  }
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-messages-area {
      ...
      overflow-x: hidden;
      overflow-y: auto;
      padding: calc(66px + env(safe-area-inset-top)) 20px calc(118px + env(safe-area-inset-bottom));
    ...
    .chat-screen.bubble-mode .message-row.assistant {
      justify-content: flex-start;
      align-items: flex-start;
    }

    .chat-screen.bubble-mode .message-row.user {
      justify-content: flex-end;
      align-items: flex-start;
    }

    .chat-screen.bubble-mode .message-row.assistant .avatar-box {
      margin-top: 24px;
    }

    .chat-screen.bubble-mode .message-row.user .avatar-box {
      margin-top: 2px;
    }

    .chat-screen.bubble-mode .message-row.assistant .message-body {
      align-items: flex-start;
    }

    .chat-screen.bubble-mode .message-row.user .message-body {
    ...
    .claude-fold-card summary {
      list-style: none;
      cursor: pointer;
    }

    .claude-fold-card summary::-webkit-details-marker {
      display: none;
    }
    ...
    .claude-fold-card[open] .fold-arrow {
    ...
    .tool-status-running .fold-icon {
      color: var(--accent-dark);
      background: var(--accent-light);
    }

    .tool-status-done .fold-icon,
    .tool-status-error .fold-icon {
      color: var(--accent-dark);
      background: var(--bg-card);
    }

    .tool-meta-line + .tool-meta-line {
    ...
    .chat-screen.dialog-mode .chat-message-list {
      max-width: 720px;
    }

    .chat-screen.dialog-mode .message-row,
    .chat-screen.dialog-mode .message-row.user,
    .chat-screen.dialog-mode .message-row.assistant {
      display: grid;
    ...
    .chat-screen.dialog-mode .message-row .avatar-box {
      margin-top: 0;
    }

    .chat-screen.dialog-mode .message-body,
    .chat-screen.dialog-mode .message-row.user .message-body,
    .chat-screen.dialog-mode .message-row.assistant .message-body {
      order: 1;
      max-width: none;
      width: 100%;
      align-items: stretch;
    ...
    .chat-screen.dialog-mode .message-name {
      display: block;
      margin-left: 2px;
    }

    .chat-screen.dialog-mode .message-actions {
      justify-content: flex-start;
    }
    ...
    .chat-details,
    .memory-editor-inline,
    .memory-floating-editor {
      padding: var(--spacing-md);
      border-radius: var(--radius-lg);
      background: var(--bg-card);
      box-shadow: var(--shadow-sm);
    ...
    .memory-editor-inline summary::-webkit-details-marker {
      display: none;
    }

    .chat-details > *:not(summary),
    .memory-editor-inline > *:not(summary) {
      margin-top: var(--spacing-md);
    }

    .memory-floating-title {
      color: var(--text-primary);
      font-size: var(--font-size-title);
      font-weight: 600;
      line-height: 1.35;
