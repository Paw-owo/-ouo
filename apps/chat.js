...
function createThinkingBlock(text) {
  ...
  const summary = document.createElement('summary');
  summary.textContent = '看看 TA 刚才怎么想';
...
function createMessageActions(message) {
  ...
  if (!isUser) {
    ...
    actions.append(play, refresh, quote, more);
    return actions;
  }

  const quote = miniAction('copy', '引用', () => quoteMessage(message));
  const edit = miniAction('edit', '编辑', () => editUserMessage(message));
...
function openMcpCallSheet(server, tool) {
  ...
  const run = button('调用并塞进上下文', 'primary', 'check');
...
async function openMemoryPanel() {
  ...
  const addButton = button('添一条小记忆', 'primary', 'add');
  addButton.addEventListener('click', () => openMemoryEditSheet(null, () => renderMemoryManager(sheet, listEl)));

  sheet.append(addButton, listEl);
  await renderMemoryManager(sheet, listEl);

  showBottomSheet(sheet);
}

async function renderMemoryManager(sheet, listEl) {
  ...
  memories.forEach((memory) => {
    listEl.appendChild(createMemoryCard(memory, () => renderMemoryManager(sheet, listEl)));
  });
}

function createMemoryCard(memory, onRefresh) {
  ...
  edit.addEventListener('click', () => openMemoryEditSheet(memory, onRefresh));

  const del = iconButton('delete', '删除记忆');
  del.addEventListener('click', () => deleteMemoryItem(memory, onRefresh));
...
function openMemoryEditSheet(memory, onRefresh) {
  const isEdit = Boolean(memory);
  const sheet = el('div');
  sheet.append(
    el('div', 'sheet-title', isEdit ? '修一下这条小记忆' : '添一条小记忆'),
    el('div', 'sheet-description', '写得短一点，TA 会更容易记住。')
  );

  const content = textarea('比如：我喜欢雨天喝热奶茶', memory?.content || '');
  const save = button(isEdit ? '保存这版' : '记住这条', 'primary', 'check');

  save.addEventListener('click', async () => {
    const saved = await saveMemoryDraft(memory, content.value, memory?.source || 'manual');
    if (!saved) return;

    hideBottomSheet();
    window.setTimeout(openMemoryPanel, 180);
  });
...
async function deleteMemoryItem(memory, onRefresh) {
