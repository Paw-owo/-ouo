...
function applyChatBackground(screen, character) {
  ...
  screen.style.backgroundImage = `linear-gradient(color-mix(in srgb, var(--bg-primary) 76%, transparent), color-mix(in srgb, var(--bg-primary) 76%, transparent)), url("${character.chatBackground}")`;
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .switch-row[data-checked="true"] .switch-track {
      background: var(--accent);
    }

    .switch-row[data-checked="true"] .switch-thumb {
