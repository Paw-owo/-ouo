...
function createHeader({ title, subtitle = "", leftText = "返回桌面", onLeftClick, rightText = "", onRightClick }) {
  const header = document.createElement("header");
  header.className = "wechat-header";

  const leftButton = createButton(leftText, "back-home-button");
  ...
  titleBox.className = "wechat-header-title";
  ...
  actions.className = "wechat-header-actions";

  if (rightText) {
    const rightButton = createButton(rightText, "secondary-button");
    rightButton.style.minHeight = "36px";
...
function renderConversationListView() {
  refreshDataFromStorage();

  const shell = document.createElement("div");
  shell.className = "wechat-shell";
  ...
  shell.appendChild(createHeader({
    ...
    rightText: chatMode === "group" ? "新建群" : "角色",
    onRightClick: () => {
      if (chatMode === "group") {
  ...
  tabs.className = "wechat-mode-tabs";

  const singleButton = createButton("私聊", `wechat-mode-tab ${chatMode === "single" ? "active" : ""}`);
  const groupButton = createButton("群聊", `wechat-mode-tab ${chatMode === "group" ? "active" : ""}`);
  ...
  tabs.appendChild(singleButton);
  tabs.appendChild(groupButton);
  shell.appendChild(tabs);

  const list = document.createElement("main");
  list.className = "wechat-conversation-list";

  const card = document.createElement("div");
  card.className = "wechat-conversation-card";

  if (chatMode === "single") {
    if (characters.length === 0) {
      card.appendChild(createEmptyState("还没有角色"));
    } else {
      characters.forEach((character) => {
        card.appendChild(createSingleConversationRow(character));
      });
    }
  } else if (groups.length === 0) {
    card.appendChild(createEmptyState("还没有群聊"));

    const createGroupButton = createButton("新建群聊", "primary-button");
    createGroupButton.addEventListener("click", showCreateGroupPanel);
    card.appendChild(createGroupButton);
  } else {
    groups.forEach((group) => {
      card.appendChild(createGroupConversationRow(group));
    });
  }
...
function createSingleConversationRow(character) {
  const row = document.createElement("button");
  row.type = "button";
  row.className = "wechat-conversation-row";

  const history = Array.isArray(character.chatHistory) ? character.chatHistory.filter((message) => !message.hidden) : [];
  const lastMessage = history[history.length - 1];

  const main = document.createElement("div");
  main.className = "wechat-conversation-main";

  const top = document.createElement("div");
  top.className = "wechat-conversation-top";

  const name = document.createElement("div");
  name.className = "wechat-conversation-name";
  name.textContent = character.name || "未命名角色";

  const time = document.createElement("div");
  time.className = "wechat-conversation-time";
  time.textContent = formatMessageTime(lastMessage?.createdAt);

  const preview = document.createElement("div");
  preview.className = "wechat-conversation-preview";
  preview.textContent = buildPreviewText(lastMessage, "还没有聊天，点进来说第一句话。");
  ...
  row.appendChild(createAvatar(character, 48));
...
function createGroupConversationRow(group) {
  ...
  const history = Array.isArray(group.chatHistory) ? group.chatHistory.filter((message) => !message.hidden) : [];
  const lastMessage = history[history.length - 1];
  ...
  const preview = document.createElement("div");
  preview.className = "wechat-conversation-preview";
  preview.textContent = buildPreviewText(lastMessage, `群成员 ${members.length} 位，点进群聊。`);
  ...
  row.appendChild(createAvatar(group, 48));
  row.appendChild(main);
...
function buildPreviewText(message, emptyText) {
  if (!message) return emptyText;

  const prefix = message.role === "assistant" ? "" : "我：";

  if (message.image) {
    return `${prefix}图片 · ${message.content || ""}`.trim();
  }

  return `${prefix}${message.content || "消息"}`;
}
...
function renderRoomView() {
  ...
  renderSingleChatMain();
}

function renderSingleChatMain() {
  refreshDataFromStorage();
  ...
  const shell = document.createElement("div");
  shell.className = "wechat-shell wechat-room";

  if (character.chatBackground) {
    shell.style.backgroundImage = `linear-gradient(rgba(255,250,253,0.72), rgba(247,239,246,0.72)), url("${character.chatBackground}")`;
  }

  shell.appendChild(createHeader({
    title: character.name || "未命名角色",
    subtitle: buildSingleStatusText(character),
    leftText: "消息",
    onLeftClick: async () => {
      if (isSending) {
        const confirmed = await showConfirm("当前回复还没结束，确定返回消息列表吗？");
        if (!confirmed) return;
      }
  ...
  topTools.className = "wechat-mode-tabs";

  const configButton = createButton("配置", "wechat-mode-tab");
  configButton.addEventListener("click", () => showApiInfoPanel(character));

  const memoryButton = createButton("记忆", "wechat-mode-tab");
  memoryButton.addEventListener("click", showSingleMemoryPanel);

  topTools.appendChild(configButton);
  topTools.appendChild(memoryButton);
  shell.appendChild(topTools);
  ...
  shell.appendChild(createInputArea("输入消息", sendUserMessage));
...
function renderGroupChatMain() {
  ...
  shell.appendChild(createHeader({
    ...
    subtitle: `群成员 ${members.length} 位 · 记忆 ${Array.isArray(group.memories) ? group.memories.length : 0} 条`,
    leftText: "消息",
    onLeftClick: async () => {
      if (isSending) {
        const confirmed = await showConfirm("当前回复还没结束，确定返回消息列表吗？");
  ...
  const manageButton = createButton("群设置", "wechat-mode-tab");
  manageButton.addEventListener("click", showGroupManagePanel);

  const speakerButton = createButton("指定回复", "wechat-mode-tab");
  speakerButton.addEventListener("click", showGroupSpeakerPicker);

  topTools.appendChild(manageButton);
  topTools.appendChild(speakerButton);
  shell.appendChild(topTools);
  ...
  shell.appendChild(createInputArea("在群聊中发言", sendGroupUserMessage));
...
function createMessageElement({ message, index, avatarEntity, speakerName, speakerCharacter, menuType }) {
  ...
  bubble.className = `message-bubble ${message.role === "assistant" ? "ai" : "user"}`;
...
function updateStatus(text) {
  const status = document.querySelector(".wechat-header-title span");
  if (!status) return;
...
async function requestGroupAiReply(characterId, options = {}) {
  ...
  isSending = true;
  updateSendButton();
  updateStatus(`${speaker.name || "群成员"} 正在回复中`);
...
function showGroupManagePanel() {
  ...
  deleteButton.addEventListener("click", async () => {
    ...
    currentGroupId = groups[0]?.id || "";
    setActiveGroupId(currentGroupId);
    currentView = groups.length ? "room" : "list";
...
function showApiInfoPanel(character) {
  ...
  info.innerHTML = `
    <div style="font-weight:800;margin-bottom:8px;">当前使用配置</div>
  ...
  desc.innerHTML = `
    <div style="font-weight:800;margin-bottom:8px;">说明</div>
    <div style="line-height:1.8;color:var(--text-secondary);font-size:13px;">
...
export function mountApp({ root, close }) {
  rootElement = root;
  closeApp = typeof close === "function" ? close : null;
  currentView = "list";
