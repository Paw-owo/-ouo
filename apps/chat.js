...
function renderChatScreen() {
  ...
  chatSearchInput.className = 'input-card';
...
function toggleChatSearchBar(screen) {
  ...
  bar.querySelector('.input-card')?.focus();
}
...
function closeChatSearchBar(screen) {
  ...
  const inputEl = bar.querySelector('.input-card');
...
function createMessageRow(message) {
  ...
  if (message.thinking) bubble.appendChild(createThinkingBlock(message.thinking));

  if (Array.isArray(message.toolCalls) && message.toolCalls.length) {
    message.toolCalls.forEach((toolCall) => bubble.appendChild(createToolCallCard(toolCall)));
  }
...
function createThinkingBlock(text) {
  ...
  main.append(
    ...
    el('span', 'fold-text', summarizeThinking(normalized))
  );

  const arrow = el('span', 'fold-arrow');
  arrow.appendChild(createIcon('arrow-down', 16));

  const content = el('div', 'fold-content thinking-content', normalized || 'TA 正在整理思路');
...
function createToolCallCard(toolCall) {
  ...
  main.append(
    ...
    el('span', 'fold-text', toolCall.serverName || '系统小工具')
  );
  ...
  content.append(
    createToolMetaLine('名称', toolCall.toolName || ''),
    createToolMetaLine('内容', stringifyCompact(toolCall.result || toolCall.arguments))
  );
...
function createToolCallSummary(toolCall) {
  if (toolCall.status === 'running') return `正在处理：${toolCall.toolName || '小工具'}`;
  if (toolCall.status === 'error') return `处理失败：${toolCall.toolName || '小工具'}`;
  return `处理完成：${toolCall.toolName || '小工具'}`;
}
...
async function generateGroupAssistantReply(character) {
  ...
  const systemPrompt = await buildSystemPrompt(character, config, '你正在群聊里回复。可以自然提到其他成员刚才说的话，但不要替别人说话。');
...
function createMemoryCard(memory, listEl) {
  ...
  del.addEventListener('click', async () => {
    ...
    await appendMemoryToolMessage(currentCharacter.id, { ...memory, content: `删除记忆：${memory.content}` }, '编辑记忆');
    await renderMemoryManager(listEl);
    renderChatScreen();
  });
