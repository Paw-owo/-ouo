...
async function sendGroupMessage(rawText, extra = {}) {
  const text = String(rawText || '').trim();
  if (!text || isSending || !currentGroup) return;
...
async function buildRelationshipPrompt(characterId) {
  try {
    ...
    const firstTime = messages[0]?.timestamp ? new Date(messages[0].timestamp).getTime() : Date.now();
...
function buildGroupChatMessages(messages, member) {
  return normalizeArray(messages)
    ...
    .filter((item) => item.role === 'user' || item.role === 'assistant')
    .map((item) => {
      const name = item.role === 'user' ? getCurrentUserDisplayProfile().name || '用户' : getSpeakerName(item.characterId);
      return {
...
function resolveCharacterEndpointId(character) {
  if (!character?.apiConfig || character.apiConfig.useGlobal !== false) {
    return '';
  }
...
function openToolboxSheet() {
  const sheet = el('div', 'toolbox-sheet');
...
function openTransferSheet() {
  const sheet = el('div', 'transfer-sheet');
  ...
  if (currentGroup) {
    ...
    sheet.appendChild(formRow('收款对象', targetSelect));
  }
...
async function openMcpConfigSheet() {
  ...
  const sheet = el('div', 'mcp-config-sheet');
  const head = el('div', 'sheet-head');
...
function openMessageActions(message) {
  const sheet = el('div', 'message-action-sheet');
...
async function editUserMessage(message) {
  const sheet = el('div', 'edit-message-sheet');
  const head = el('div', 'sheet-head');
...
async function renderMemoryPage(page) {
  ...
  nav.append(back, el('div', 'nav-title', '记忆'), iconButton('add', '新增记忆'));

  const addButton = nav.querySelector('button:last-child');
  addButton.addEventListener('click', () => openMemoryEditSheet(null, page));
...
function openMemoryEditSheet(memory, page) {
  const sheet = el('div', 'memory-edit-sheet');
  const head = el('div', 'sheet-head');
...
async function maybeSendProactiveMessage(character, source = 'scan') {
  ...
  if (config.proactiveMode1Enabled && source === 'scan') {
    ...
    if (
      ...
      return sendProactiveMessage(character, '用户已经一段时间没回复你，请结合时间段和上下文自然主动发一条消息，不要像提醒机器人。');
    }
  }
  ...
  if (config.proactiveMode2Enabled && source === 'active' && currentCharacter?.id === character.id) {
    ...
    return sendProactiveMessage(character, '用户停留在聊天界面但暂时没说话，请结合上下文自然开口，不要尬聊。');
  }
...
async function openGroupCreateSheet() {
  ...
  const sheet = el('div', 'group-create-sheet');
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-app .message-text-part {
      white-space: pre-wrap;
    }

    .chat-app .code-fold-card {
      margin: 6px 0;
      background: color-mix(in srgb, var(--bg-primary) 64%, var(--bg-card));
    }
    ...
    .chat-app .toolbox-sheet,
    .chat-app .chat-config-sheet,
    .chat-app .mcp-config-sheet,
    .chat-app .transfer-sheet,
    .chat-app .sticker-sheet,
    .chat-app .message-action-sheet,
    .chat-app .edit-message-sheet,
    .chat-app .memory-edit-sheet,
    .chat-app .group-create-sheet,
    .chat-app .group-settings-sheet,
    .bottom-sheet .toolbox-sheet,
    .bottom-sheet .chat-config-sheet,
    .bottom-sheet .mcp-config-sheet,
    .bottom-sheet .transfer-sheet,
    .bottom-sheet .sticker-sheet,
    .bottom-sheet .message-action-sheet,
    .bottom-sheet .edit-message-sheet,
    .bottom-sheet .memory-edit-sheet,
    .bottom-sheet .group-create-sheet,
    .bottom-sheet .group-settings-sheet {
      display: flex;
      flex-direction: column;
      gap: 14px;
      padding: 4px 0 10px;
