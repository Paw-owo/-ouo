...
function createSingleMessageRow(message, options = {}) {
  ...
  if (isUser) {
    if (message.type === 'image' && message.imageBase64) {
      ...
      body.appendChild(createTransferCard(message.transferAmount, message.transferTargetId));
    } else {
...
async function replyGroupMembers(extraInstruction = '') {
  if (!currentGroup || isSending) return;
  ...
  try {
    ...
    for (const character of speakers) {
      ...
      await generateGroupAssistantReply(character, extraInstruction);
    }
  } finally {
    isSending = false;
    renderChatScreen();
...
async function generateGroupAssistantReply(character, extraInstruction = '') {
...
function openToolSheet() {
  ...
  if (!currentGroup) {
    pageTwoItems.push({ icon: 'memory', label: '记忆系统', action: openMemoryPage });
  }

  const pages = el('div', 'tool-pages');

  const pageOne = el('div', 'tool-page');
  pageOne.appendChild(createToolGrid(pageOneItems));

  const pageTwo = el('div', 'tool-page');
  pageTwo.appendChild(createToolGrid(pageTwoItems));
  ...
  file.addEventListener('change', async () => {
    ...
    try {
      ...
      if (currentGroup) {
        ...
        await updateLatestGroupCache(currentGroup.id);
        renderChatScreen();
        await replyGroupMembers('用户刚刚在群聊里发了一张图片。你无法直接看见图片细节，只能根据用户之后补充的信息自然回应。');
        return;
      }
...
function openStickerPanel() {
  const bound = currentGroup
    ? getGroupStickers()
    : stickers.filter((item) => currentCharacter?.stickerIds?.includes(item.id));

  const sheet = el('div');
  sheet.appendChild(el('div', 'sheet-title', '表情包'));

  if (!bound.length) {
    sheet.appendChild(createSoftNote(currentGroup ? '群成员还没有绑定表情包。' : '这个角色还没有绑定表情包。'));
    showBottomSheet(sheet);
    return;
  }
  ...
  function render(filter = '') {
    ...
    bound
      .filter((sticker) => {
        ...
        const text = `${sticker.description || ''} ${(sticker.tags || []).join(' ')} ${sticker.ownerName || ''}`.toLowerCase();
      ...
      .forEach((sticker) => {
        ...
        item.addEventListener('click', async () => {
          ...
          if (currentGroup) {
            ...
            await updateLatestGroupCache(currentGroup.id);
            renderChatScreen();
            await replyGroupMembers('用户刚刚在群聊里发了一个表情，请结合表情描述自然回应。');
            return;
          }
...
function getGroupStickers() {
  ...
  characters
    ...
    .forEach((character) => {
      normalizeArray(character.stickerIds).forEach((stickerId) => {
...
function openCallUI() {
  ...
  const isGroup = Boolean(currentGroup);
  const callTitle = isGroup ? currentGroup.name || '群聊' : currentCharacter.name || 'AI';
  const callAvatar = isGroup ? currentGroup.avatar : currentCharacter.avatar;

  const call = el('section', 'call-screen');
  if (!isGroup) applyChatBackground(call, currentCharacter);

  const top = el('div', 'call-time', '00:00');
  const avatar = createAvatar(callAvatar, callTitle, 'call');
  const name = el('div', 'call-name', callTitle);
  const status = el('div', 'call-status', isGroup ? '群聊文字通话中，内容会保存到群聊' : '文字通话中，内容会保存到聊天记录');
  ...
  async function sendCallMessage() {
    ...
    if (isGroup) {
      await sendGroupCallMessage(content, log, avatar);
      return;
    }

    await sendPrivateCallMessage(content, log, avatar);
  }
...
async function sendPrivateCallMessage(content, log, avatar) {
  if (!currentCharacter) return;
...
async function sendGroupCallMessage(content, log, avatar) {
  if (!currentGroup) return;
  ...
  const members = shuffleArray(normalizeArray(currentGroup.memberIds))
    .map((id) => characters.find((character) => character.id === id))
    .filter(Boolean)
    .slice(0, 2);

  if (!members.length) {
    log.appendChild(el('div', 'call-line ai', '群里还没有能回复的角色。'));
    return;
  }
  ...
  for (const character of members) {
    ...
    const systemPrompt = await buildSystemPrompt(
      character,
      config,
      `你正在群聊「${currentGroup.name || '群聊'}」的文字通话里回复。你只能扮演自己：${character.name || 'AI'}。回复要口语、短一点、适合朗读。`
    );

    const callMessages = currentMessages.slice(-18).map((message) => ({
      role: message.role === 'assistant' ? 'assistant' : 'user',
      content: `${getSpeakerName(message.characterId)}：${getMessagePreview(message, true)}`
    }));
    ...
    const line = el('div', 'call-line ai', `${character.name || 'AI'}：${final}`);
    log.appendChild(line);

    avatar.classList.add('speaking');
    stopActiveTts();
    activeTts = playTTS(final, character.ttsConfig);
    activeTtsMessageId = assistantMessage.id;
...
async function sendProactiveMessage(character, mode, instruction) {
  ...
  message.autoVoice = false;
  message.voiceAutoPlaying = false;
...
function openTransferSheet() {
  ...
  if (currentGroup) {
    normalizeArray(currentGroup.memberIds)
      ...
      .forEach((character) => {
        ...
        option.textContent = character.name || 'AI';
  ...
  send.addEventListener('click', async () => {
    ...
    if (currentGroup) {
      const targetId = targetSelect.value || normalizeArray(currentGroup.memberIds)[0] || '';
      const targetName = getSpeakerName(targetId);

      const message = createMessage({
        role: 'user',
        content: `给${targetName}转了 ${value.toFixed(2)} 元`,
        characterId: 'user',
        groupId: currentGroup.id,
        type: 'transfer',
      ...
      message.transferTargetId = targetId;
      ...
      hideBottomSheet();
      renderChatScreen();
      await replyGroupMembers(`用户刚刚给${targetName}转了 ${value.toFixed(2)} 元。收款者要自然回应，其他成员可以围观或调侃。`);
      return;
    }
  ...
  if (currentGroup) {
    sheet.append(field('收款角色', targetSelect));
  }
...
function createTransferCard(amount, targetId = '') {
  ...
  card.append(
    el('div', 'transfer-title', targetId ? `转账给 ${getSpeakerName(targetId)}` : '转账'),
...
function getMessagePreview(message, full = false) {
  ...
  if (message.type === 'transfer') {
    const target = message.transferTargetId ? `给${getSpeakerName(message.transferTargetId)} ` : '';
    return `${target}转账 ${Number(message.transferAmount || 0).toFixed(2)} 元`;
  }
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .message-row.assistant-split-row {
      gap: 8px;
      margin-top: -8px;
    }

    .message-row.assistant-split-row + .message-row.assistant-split-row {
      margin-top: -12px;
    }

    .message-row.assistant-split-row .message-name {
      margin-bottom: 5px;
    }

    .message-row.assistant-split-row .message-bubble {
    ...
    .message-row.assistant-split-row .message-actions {
      margin-top: 7px;
    }
