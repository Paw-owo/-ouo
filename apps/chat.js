...
function createMemoryToolStatusLine(message) {
  ...
  row.append(
    createToolStatusIcon(first.status || 'done'),
...
function createThinkingBlock(text, timeMs) {
  ...
  const left = el('div', 'thinking-left');
  left.append(
    createIcon('star', 16),
    el('span', 'thinking-title', '思考'),
  ...
  summary.append(left, summaryText, arrow);
...
function createToolChainBlock(toolCalls) {
  ...
  toolCalls.forEach((toolCall, index) => {
    if (index > 0) {
      const connector = el('div', 'tool-chain-connector');
      connector.appendChild(el('div', 'tool-chain-line'));
...
function createToolCallCard(toolCall) {
  ...
  const icon = createToolStatusIcon(toolCall.status);
...
function createToolStatusIcon(status) {
  const iconName = status === 'running' ? 'refresh' : status === 'error' ? 'clear' : 'check';
  const box = el('span', 'tool-status-icon');
  box.appendChild(createIcon(iconName, 16));
  return box;
}
...
function getToolCallDesc(toolCall) {
  if (isMemoryToolCall(toolCall)) return '记忆系统';
...
function createMemoryCard(memory, listEl) {
  ...
  del.addEventListener('click', async () => {
    ...
    await appendMemoryToolMessage(currentCharacter.id, { ...memory, content: '' }, '编辑记忆');
    await renderMemoryManager(listEl);
    renderChatScreen();
  });
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .thinking-block summary {
      ...
      cursor: pointer;
      display: grid;
      grid-template-columns: minmax(0, 1fr) minmax(0, 2.2fr) 18px;
      align-items: center;
      gap: 8px;
      padding: 6px 2px;
    }
    ...
    .tool-call-card summary {
      ...
      cursor: pointer;
      display: grid;
      grid-template-columns: 20px minmax(0, 1fr) minmax(0, 1.4fr) 18px;
      align-items: center;
      gap: 8px;
      padding: 6px 0;
    }
    ...
    @media (max-width: 420px) {
      ...
      .thinking-block summary {
        grid-template-columns: minmax(0, 1fr) minmax(0, 1.6fr) 18px;
      }

      .tool-call-card summary {
        grid-template-columns: 20px minmax(0, 1fr) minmax(0, 1fr) 18px;
      }
    }
  `;
