...
function createMessageRow(message) {
  ...
  if (!isUser && bubbleMode && shouldSplitAssistantBubbles(message)) {
    return createAssistantBubbleFragment(message);
  }
...
function shouldSplitAssistantBubbles(message) {
  ...
  return text.length > 22 && !text.includes('```');
}

function createAssistantBubbleFragment(message) {
  const fragment = document.createDocumentFragment();
  const chunks = splitAssistantBubbleText(message.content);
  const speakerName = getSpeakerName(message.characterId);
  const speakerAvatar = getSpeakerAvatar(message.characterId);

  chunks.forEach((chunk, index) => {
    const row = el('article', 'message-row assistant assistant-split-row');
    row.dataset.messageId = index === 0 ? message.id : `${message.id}_${index}`;
    row.dataset.parentMessageId = message.id;

    const avatar = createAvatar(speakerAvatar, speakerName, 'sm');
    const body = el('div', 'message-body');

    body.appendChild(el('div', 'message-name', speakerName));
    body.appendChild(createBubbleBlock(chunk));
    ...
    const longPressTarget = body.querySelector('.message-bubble') || body;
    longPressTarget.addEventListener('pointerdown', () => {
      clearLongPress();
      longPressTimer = window.setTimeout(() => openMessageActions(message), 520);
    });
...
function splitAssistantBubbleText(text) {
  ...
  chunks.forEach((chunk) => {
    ...
    while (rest.length > 24) {
      let cut = Math.min(20, rest.length);
      ...
      if (punctuationIndex >= 10) cut = punctuationIndex + 1;
...
function renderMessagePatch(message) {
  ...
  if (row && shouldSplitAssistantBubbles(message)) {
    renderChatScreen();
    return;
  }
