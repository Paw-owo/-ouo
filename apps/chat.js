...
async function renderCurrentRoute() {
  ...
  if (currentRoute.name === 'thread') {
    await mountChatThread(rootEl, {
      ...
      characterId: currentRoute.params.characterId,
      groupId: currentRoute.params.groupId
    });
    return;
  }

  if (currentRoute.name === 'memory') {
    await mountChatMemory(rootEl, {
      appState,
      characterId: currentRoute.params.characterId,
      fromRoute: currentRoute.params.fromRoute
    });
    return;
  }

  await mountChatList(rootEl, {
    appState,
    tab: currentRoute.params.tab,
    search: currentRoute.params.search
  });
}
...
// 改了什么：恢复 recordExternalInteraction({characterId, role, content, source}) 对象入参，并兼容旧的 recordExternalInteraction(characterId, interaction)；外部互动只写入 memories，不恢复聊天入口、不写 messages、不加未读。
// 会不会影响其他文件：不会要求其他文件同步更新；外部应用按新旧两种方式调用都可用。
