...
// 上下文感知：根据当前时间段（早/午/晚/深夜）返回不同回复；
//             读取好感度，好感度高（>=60）时回复更亲密。
// 依赖：core/affection.js（仅读缓存，同步）
...
/** 根据当前小时返回时段：morning/afternoon/evening/night */
...
/**
 * 一次性拿本地回复（关键词分类 + 时段 + 好感度 + 去重），方便调用方直接用。
