...
function createMemoryHero() {
  ...
  const aiAvatar = createPairAvatar(state.character?.avatar || '', state.character?.name || 'TA', 'ai');
  const userAvatar = createPairAvatar(user.avatar || '', user.name || '我', 'user');

  pair.append(aiAvatar, userAvatar);
...
function createPairAvatar(src, name = '', type = '') {
...
function createCharacterPicker() {
  const wrap = el('section', 'chat-memory-character-picker');

  if (!state.characters.length) {
    wrap.appendChild(el('div', 'chat-memory-character-empty', '还没有角色可以记东西'));
    return wrap;
  }

  const select = document.createElement('select');
  select.className = 'chat-memory-character-select';
  ...
  select.addEventListener('change', async () => {
    state.characterId = select.value;
    state.editingId = '';
    await loadData();
    render();
  });
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-memory-pair-avatar.ai {
      left: 0;
      z-index: 2;
    }

    .chat-memory-pair-avatar.user {
      right: 0;
      z-index: 1;
...
// 改了什么：记忆页顶部改成 AI 和用户双头像贴在一起，删掉原角色头像横条，记忆卡片按内容自动变宽，并修正 memories 写库调用。
// 会不会影响其他文件：不会。
// 更新记忆里该文件的导出函数：无变化。
