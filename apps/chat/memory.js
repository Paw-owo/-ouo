...
function createHeroAvatarBlock({ type, src, name, label }) {
  const block = el('div', `chat-memory-hero-block ${type}`);

  const tag = el('div', 'chat-memory-hero-tag', label || name || '');
...
// 改了什么：双头像改成参考图那种并排轻贴的大圆头像，并修复用户头像读取字段。
// 会不会影响其他文件：不会。
// 更新记忆里该文件的导出函数：无变化。
