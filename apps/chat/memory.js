// apps/chat/memory.js
// imports:
//   from '../../core/storage.js': generateId, getNow, getAllDB, getDB, setDB, deleteDB, getByIndexDB
...
function createCharacterStrip() {
  const wrap = el('section', 'chat-memory-character-strip');

  if (!state.characters.length) {
    const empty = el('div', 'chat-memory-character-empty', '还没有角色可以记东西');
    wrap.appendChild(empty);
    return wrap;
  }

  state.characters.forEach((character) => {
    const button = el('button', `chat-memory-character ${character.id === state.characterId ? 'active' : ''}`);
    button.type = 'button';

    button.append(
      createAvatar(character.avatar, character.name),
      el('span', 'chat-memory-character-name', character.name || '未命名')
    );

    button.addEventListener('click', async () => {
      if (state.characterId === character.id) return;
...
async function saveMemory(memory) {
  await setDB('memories', memory.id, normalizeMemory(memory));
}
...
function normalizeMemory(memory) {
  ...
  return {
    id: String(memory.id || generateId()),
...
function createAvatar(src, name = '') {
  const avatar = el('span', 'chat-memory-avatar');
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-memory-character-strip::-webkit-scrollbar,
    .chat-memory-list::-webkit-scrollbar {
      display: none;
    }

    .chat-memory-character {
      flex: 0 0 auto;
      min-height: 44px;
      display: inline-flex;
      align-items: center;
    ...
    .chat-memory-character.active .chat-memory-avatar {
      background: color-mix(in srgb, var(--bubble-user-text) 18%, transparent);
      color: var(--bubble-user-text);
    }

    .chat-memory-avatar img {
      width: 100%;
      height: 100%;
      object-fit: cover;
    }

    .chat-memory-character-name {
      max-width: 108px;
      overflow: hidden;
...
// 改了什么：只统一了记忆保存的 source/updatedAt 格式，没动其他结构。
// 会不会影响其他文件：不会。
// 更新记忆里该文件的导出函数：mountChatMemory(containerEl, options) / unmountChatMemory()
// 依赖：../../core/storage.js(generateId,getNow,getAllDB,getDB,setDB,deleteDB,getByIndexDB)；../../core/ui.js(createIcon,showToast,showConfirm)
