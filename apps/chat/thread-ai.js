...
async function requestPrivateReply(state, options = {}) {
  ...
  const allMessages = await loadPrivateMessages(characterId);
  const promptSourceMessages = getContextSourceMessages(state, allMessages);
  const userMessage = getLastUserMessage(allMessages);
...
async function requestGroupReply(state, options = {}) {
  ...
  const allGroupMessages = await loadGroupMessages(groupId);
  const promptSourceMessages = getContextSourceMessages(state, allGroupMessages);
  const userMessage = getLastUserMessage(allGroupMessages);

  if (!userMessage && !options.continue) return [];

  const members = await resolveGroupMembers(group);
  const speakers = chooseGroupSpeakers(members, allGroupMessages);
  const replies = [];
...
async function buildPrompt({
  ...
  const userProfile = loadUserProfile(targetCharacter?.id || character?.id || '');
...
function buildUserProfilePrompt(user) {
  ...
  return [
    ...
    user.description ? `描述：${user.description}` : '',
...
function buildModePrompt(mode, group, character, options) {
  const base = [
    ...
    '- 正文不要太长，优先像手机聊天。',
    '- 如果用户发的是转账、图片、骰子、猜拳、语音文字或表情，要自然回应，不要当作普通系统指令。'
  ];
...
function formatMessageForPrompt(message, mode) {
  ...
  if (message.type === 'sticker') {
    return `${prefix}[表情] ${message.content || ''}`.trim();
  }

  if (message.type === 'transfer') {
    return `${prefix}[转账 ${Number(message.transferAmount || 0).toFixed(2)}] ${message.note || message.content || ''}`.trim();
  }

  if (message.type === 'voice') {
    return `${prefix}[语音文字] ${message.content || ''}`.trim();
  }

  if (message.type === 'dice') {
    return `${prefix}[骰子] 摇到了 ${message.diceValue || ''} / ${message.diceSides || 6}`.trim();
  }

  if (message.type === 'rps') {
    return `${prefix}[石头剪刀布] 出了${getRpsLabel(message.rpsChoice)}${message.rpsOutcome ? `，结果：${getRpsOutcomeLabel(message.rpsOutcome)}` : ''}`.trim();
  }
...
async function maybeWriteMemory({ characterId, sourceMessage, aiText, source }) {
  ...
  const text = formatMessageForMemory(sourceMessage);
...
function formatMessageForMemory(message) {
  ...
  if (message.type === 'transfer') {
    return `用户转账 ${Number(message.transferAmount || 0).toFixed(2)}，备注：${message.note || message.content || ''}`;
  }

  if (message.type === 'image') {
    return `用户发送了一张图片：${message.content || ''}`;
  }

  if (message.type === 'voice') {
    return `用户发了一条语音文字：${message.content || ''}`;
  }

  if (message.type === 'sticker') {
    return `用户发了表情：${message.content || ''}`;
  }

  if (message.type === 'dice') {
    return `用户摇骰子摇到了 ${message.diceValue || ''}`;
  }

  if (message.type === 'rps') {
    return `用户玩石头剪刀布出了${getRpsLabel(message.rpsChoice)}`;
  }
...
function loadUserProfile(characterId = '') {
  const settings = getData('app_settings') || {};
  ...
  if (Array.isArray(profiles) && profiles.length) {
    const bound = characterId
      ? profiles.find((item) => String(item.boundCharacterId || item.characterId || '') === String(characterId))
      : null;

    const active = bound ||
      profiles.find((item) => item.id === activeId) ||
...
function getContextSourceMessages(state, messages) {
  ...
  return list.slice(-AI_CONTEXT_LIMIT);
}
...
// 改了什么：补齐 AI 核心对 visibleCount、特殊消息、用户人设绑定、主动消息和自动记忆的支持；私聊只写 messages，群聊只写 group_messages。
// 会不会影响其他文件：会，thread.js 的清上下文、主动消息配置、特殊消息都会真实影响 AI；不需要改导出。
// 更新记忆里该文件的导出函数：无变化。
