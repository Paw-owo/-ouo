...
function shouldRemember(text) {
  ...
  return ['我喜欢', '我不喜欢', '我讨厌', '我害怕', '我想要', '我希望', '记住', '别忘', '生日', '纪念日', '今天', '以后', '名字', '住在', '工作', '学校', '朋友', '家人']
...
function buildMemoryText(userText, aiText, userName = '你') {
  ...
  if (!user) return '';
  return ai ? `我记得${userName}提到过：${user}。我当时是这样回应的：${ai}` : `我记得${userName}提到过：${user}`;
}
...
async function loadMemories(characterId) {
  ...
  return normalizeList(list).sort(sortByUpdatedAtDesc).slice(0, MEMORY_LIMIT);
}
...
// 改了什么：新增 stopThreadAIReply，AI 等待占位写入 isPending/status，停止后写 isStopped，不再被后续请求覆盖。
// 会不会影响其他文件：会，下一步需要改 apps/chat/thread-actions.js 接入 stopThreadAIReply，再改 apps/chat/thread.js 让发送按钮变停止按钮。
// 更新记忆里该文件的导出函数：requestThreadAIReply(state,options) / stopThreadAIReply(state,options) / checkThreadProactiveMessages(state,options) / requestProactiveThreadMessage(state,reason)
