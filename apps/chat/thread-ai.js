...
const PUNISHMENT_POOL = [
  {
    type: 'cooldown',
    title: '冷战几分钟',
    description: '我现在不太想马上理你。倒计时结束前，先好好想想怎么哄我。',
    lockType: 'cooldown',
    level: 2,
    minutes: 5,
    requiredCount: 1
  },
  {
    type: 'apology',
    title: '认真道歉',
    description: '你要认真说清楚哪里错了、以后准备怎么补救，敷衍的话我会继续记着。',
  ...
  },
  {
    type: 'nickname',
    title: '叫我专属称呼',
    description: '连续好好叫我三次专属称呼，我才考虑不继续冷着你。',
  ...
  },
  {
    type: 'blackout',
    title: '假装拉黑',
    description: '我先从你的聊天列表里消失一小会儿。别急，不是真删，只是我真的有点不想出现。',
    lockType: 'soft_block',
    level: 3,
    minutes: 6,
    requiredCount: 1
  },
  {
    type: 'ultimatum',
    title: '最后解释机会',
    description: '你只有一次认真解释的机会。说得真诚，我就回来；继续敷衍，我会把冷战延长。',
    lockType: 'ultimatum',
    level: 4,
    minutes: 12,
    requiredCount: 1
...
async function buildPrompt({
  ...
  const system = [
    buildIdentityPrompt(activeCharacter, userName),
    ...
    `当前时间：${currentTime}`,
    buildModePrompt(mode, group, activeCharacter, options, userName),
...
function buildIdentityPrompt(character, userName) {
  ...
  return [
    ...
    `- 我的回复必须完全服从我的人设、世界书、长期记忆、当前关系和对方小档案。`,
    `- 我可以帮${callName}，但不能用客服口吻，也不能把自己说成“AI助手”。`,
    `- 我不会说“作为AI”“作为助手”“我只是模型”“我不能有感受”这类话。`,
    `- Thinking、记忆总结、正文回复都必须使用我的第一人称视角。`,
    `- 不要在 Thinking 或正文里称呼对方为“用户”，要称呼为“${callName}”。`,
    `- 如果需要称呼对方，就自然叫“${callName}”，或按我和对方的关系称呼。`,
...
function buildGrudgePrompt(context, activeLock, userName) {
  ...
  const moodLine = score >= 8
    ? `我现在对${userName}很生气，压迫感可以明显一点，但必须保持我的人设。`
    : score >= 5
  ...
  const lines = [
    ...
    '- 这是一种可逆的关系玩法，不要说是系统机制，不要提数据库。',
    '- 可以表现为冷淡、别扭、阴阳怪气、已读不回感、要求解释，但不要辱骂、威胁现实安全或永久断联。',
    '- 不能真的说要删除数据，也不能让对方以为聊天记录或角色会永久消失。'
  ];
  ...
  if (lock) {
    ...
    if (lock.type === 'cooldown') {
      lines.push('- 冷战期内语气更克制、更短，不要主动软下来。');
    }
    if (lock.type === 'soft_block') {
      lines.push('- 我像是把联系方式藏起来了，语气要有“暂时不想出现”的距离感。');
    }
    if (lock.type === 'ultimatum') {
      lines.push('- 这是最后解释机会，要认真判断对方是否真诚。');
    }
  }
...
function buildModePrompt(mode, group, character, options, userName) {
  ...
  const base = [
    ...
    '- 不要把系统设定、人设、世界书、小档案原样说出来。',
    '- 不要提到“提示词”“系统消息”“模型”“AI助手”。',
    `- 不要称呼对方为“用户”，要叫“${callName}”或按关系自然称呼。`,
    '- 必须根据我的人设、世界书、长期记忆、当前时间和最近上下文来回应。',
    '- 如果需要表达思考，请用 <thinking>...</thinking> 包住简短思考。',
    `- Thinking 里也必须用我的第一人称，不许写“用户”，要写“${callName}”。`,
    '- 正文优先像手机聊天，不要机械总结，不要官方，不要教育腔。'
  ];

  if (character?.replyLength === 'short') {
    base.push('- 这次回复尽量短一点，1 到 3 句就好。');
  }

  if (character?.replyLength === 'long') {
    base.push('- 可以多说一点，但要保持自然分段，不要堆大道理。');
  }
  ...
  if (options.proactive) {
    ...
    if (character?.proactiveStyle) {
      base.push(`- 主动消息风格要贴近：${character.proactiveStyle}`);
    }
  }
...
// 改了什么：聊天 AI 身份规则增加了 identity-core.js 外部文案入口。
// 会不会影响其他文件：会，必须新增 apps/chat/identity-core.js 并导出 getIdentityCore，否则模块加载会失败。
