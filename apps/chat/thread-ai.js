...
// 改了什么：删掉 thread-ai.js 自己的记忆写入和记忆加载，统一改成调用 core/memory.js，并修了思考标签和禁 emoji。
// 会不会影响其他文件：会，后面 apps/chat/thread.js 的配置抽屉可以直接加 memoryInjectLimit / memoryCandidateLimit 两个设置。
// 依赖：../../core/storage.js(getData,setData,generateId,getNow,setDB,deleteDB,getByIndexDB,getAllDB)；../../core/api.js(silentRequest)；../../core/memory.js(buildMemoryPrompt,checkImportantInfo,checkAndSummarize)；./identity-core.js(getIdentityCore)
