...
export function getRelationshipLockText(stateOrLock) {
  const stateLooksLikeLock = stateOrLock && (stateOrLock.endsAt || stateOrLock.reason || stateOrLock.type);
  const lock = stateLooksLikeLock ? stateOrLock : stateOrLock?.relationshipLock;
  const punishment = stateLooksLikeLock ? null : stateOrLock?.relationshipPunishment;
