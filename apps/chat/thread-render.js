...
//   from './thinking-chain.js': createThinkingChainButton, hasThinkingChain
...
import { createThinkingChainButton, hasThinkingChain } from './thinking-chain.js';
...
function createMessageRow(state, message, pageEl) {
  ...
  row.append(
    createThinkingStack(state, message, role, mode),
    body
  );
...
// ═══════════════════════════════════════
// 【思维链】用新组件替换旧的 createReasoningStack
// ═══════════════════════════════════════

function createThinkingStack(state, message, role, mode = 'bubble') {
  const stack = el('section', `chat-reasoning-stack role-${role} mode-${mode}`);

  if (message.role !== 'assistant' || !hasThinkingChain(message)) {
    stack.hidden = true;
    return stack;
  }

  const button = createThinkingChainButton(message, {
    roleName: state.character?.name || message.characterName || 'TA'
  });
