...
function createReasoningStack(message, role, mode = 'bubble') {
  ...
  const row = safeButton('chat-reasoning-peek', '查看概要');
  row.append(
    createLineIcon('thought'),
    el('span', 'chat-reasoning-peek-text', getReasoningSummary(message)),
    createLineIcon('chevron')
  );

  row.addEventListener('click', () => openReasoningSheet(message));
...
function openReasoningSheet(message) {
  ...
  steps.forEach((step, index) => {
    list.appendChild(createTimelineStep(step, index));
  });

  sheet.append(handle, top, list);
...
function createTimelineStep(step, index) {
  ...
  const detail = el('div', 'chat-timeline-detail');
  const pre = el('pre', 'chat-timeline-pre');
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-timeline-step[data-open="true"] .chat-timeline-arrow {
      transform: rotate(90deg);
    }
    ...
    .chat-timeline-step[data-open="true"] .chat-timeline-detail {
      max-height: 260px;
      opacity: 1;
      overflow-y: auto;
      padding-top: 10px;
