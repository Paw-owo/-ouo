...
export function renderThreadMessages(state, pageEl) {
  ...
  if (hiddenCount > 0 && !state.searchValue) {
    list.appendChild(createLoadMoreButton(state, pageEl, hiddenCount));
  }
...
function createLoadMoreButton(state, pageEl, hiddenCount) {
  ...
  button.addEventListener('click', () => {
    state.visibleCount = Math.min(getAllMessages(state).length, Number(state.visibleCount || 0) + PAGE_SIZE);
...
function createMessageRow(state, message, pageEl) {
  ...
  row.dataset.type = message.type || 'text';

  row.append(
    createMessageHead(state, message),
...
function createMessageHead(state, message) {
  ...
  meta.append(
    ...
    el('div', 'chat-message-time', formatTime(message.timestamp || message.createdAt))
  );
...
function createBubbleContent(message) {
  ...
  if (message.type === 'voice') {
    bubble.append(createVoiceTag(message));
  }
...
function createMessageContent(message) {
  ...
  if (message.type === 'image' && message.imageBase64) {
    ...
    if (message.content) {
      content.appendChild(createTextBlock(String(message.content || '')));
    }
  ...
  if (message.type === 'sticker') {
    content.appendChild(el('div', 'chat-message-sticker', String(message.content || '[表情]')));
    return content;
  }
  ...
  if (message.type === 'voice') {
    content.appendChild(createVoiceTextCard(message));
    return content;
  }
...
function createTransferCard(message) {
  const card = el('section', 'chat-transfer-card');
  const amount = Number(message.transferAmount || 0);

  card.append(
    el('div', 'chat-transfer-card-label', '转账消息'),
    el('div', 'chat-transfer-card-amount', `￥${formatAmount(amount)}`)
  );

  const note = String(message.note || message.content || '').trim();
  if (note) {
    card.appendChild(el('div', 'chat-transfer-card-note', note));
  }

  return card;
}

function createVoiceTextCard(message) {
  const card = el('section', 'chat-voice-card');
  card.append(
    createLineIcon('volume'),
    el('div', 'chat-voice-content', String(message.content || '这是一条语音文字。'))
  );
  return card;
}
...
function createMessageActions(state, message, pageEl) {
  ...
  actions.append(
    smallAction('copy', '复制', async () => copyThreadMessage(message)),
  ...
  if (canEditMessage(message)) {
    actions.append(smallAction('edit', '编辑', () => openEditSheet(state, message, pageEl)));
  }

  if (message.role === 'assistant') {
    actions.append(
      smallAction('refresh', '重来', async () => {
        await regenerateThreadMessage(state, message.id);
        renderThreadMessages(state, pageEl);
      }),
      smallAction('continue', '续写', async () => {
        await continueThreadMessage(state);
        renderThreadMessages(state, pageEl);
      }),
      smallAction('volume', '朗读', async () => playThreadTTS(state, message))
    );
  }

  actions.append(
    smallAction('trash', '删除', async () => {
...
function canEditMessage(message) {
  return String(message?.type || 'text') === 'text' && Boolean(String(message?.content || '').trim());
}
...
function openEditSheet(state, message, pageEl) {
  ...
  const title = el('div', 'chat-action-sheet-title', '改一下这句话');
...
function renderQuotePreview(state, pageEl) {
  ...
  const message = getAllMessages(state).find((item) => item.id === state.quotedMessageId);
...
function createVoiceTag(message) {
  const tag = el('div', 'chat-message-voice-tag');
  tag.append(createLineIcon('volume'), el('span', '', '语音文字'));
  if (message.duration) {
    tag.appendChild(el('span', '', `${message.duration}s`));
  }
  return tag;
}
...
function getAllMessages(state) {
  return state.mode === 'group'
    ? normalizeArray(state.groupMessages)
    : normalizeArray(state.messages);
}

function getVisibleMessages(state) {
  const list = getAllMessages(state);
  const q = String(state.searchValue || '').trim().toLowerCase();
  const count = Math.max(1, Number(state.visibleCount || list.length || 1));
  const visible = list.slice(Math.max(0, list.length - count));
  ...
  return visible.filter((message) => {
    const haystack = [
      ...
      message.thinking,
      normalizeArray(message.toolCalls).map((tool) => normalizeToolValue(tool)).join('\n')
    ].join('\n').toLowerCase();
...
function getPreviewText(message) {
  ...
  if (message.type === 'voice') return `[语音] ${trimOneLine(message.content || '', 40)}`;
...
function formatTime(value) {
  ...
  return `${pad(time.getMonth() + 1)}-${pad(time.getDate())} ${clock}`;
}
...
// 改了什么：补完整消息渲染层，强化操作入口、Thinking/Tool Calls、代码卡片、转账、骰子、猜拳、语音文字、表情和引用预览。
// 会不会影响其他文件：会，依赖 thread-actions.js 的消息操作函数；不需要改导出。
// 更新记忆里该文件的导出函数：无变化。
