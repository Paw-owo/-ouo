...
export function updateMessageContent(state, pageEl, messageId, content, thinking) {
  ...
  const updated = {
    ...message,
    content: String(content ?? message.content)
  };
...
function createErrorBubble(message) {
  ...
  retry.addEventListener('click', () => {
    retryThreadMessage(message).catch(() => showToast('重试没成功'));
  });
...
function createImageContent(message) {
  ...
  const wrap = el('div', 'chat-message-content');
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-message-row.mode-dialog.role-user .chat-message-author {
      margin-right: 46px;
    }

    .chat-message-row.mode-dialog.role-assistant .chat-message-author {
      margin-left: 46px;
    }
    ...
    .chat-message-body.role-assistant .chat-version-pager {
      order: 3;
    }
    ...
    .chat-version-btn:not(:disabled):active {
    ...
    .chat-tool-call-dot.status-running {
      background: var(--accent);
      animation: chatToolDotPulse 1.2s ease-in-out infinite;
    }

    .chat-tool-call-dot.status-error {
      background: var(--text-secondary);
    }

    .chat-tool-call-dot.status-done {
      background: var(--accent);
    }
