...
function createBubbleHead(state, message) {
  const head = el('div', 'chat-message-head');
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-message-row.role-user .chat-message-head {
...
// 改了什么：消息操作同一行加入 token 数，并支持显示用户上传图片表情包和 AI 可读描述。
// 会不会影响其他文件：会，thread-actions.js 里的 stickerImageBase64/stickerDescription 会被这里显示。
// 更新记忆里该文件的导出函数：无变化。
