...
export async function renderThread(ctx, options = {}) {
  ...
  const messages = createMessagesArea(ctx);
...
function createSearchBar(ctx) {
  ...
  input.addEventListener('input', () => {
    handleThreadSearch(ctx, input.value.trim());
  });
...
function createMessagesArea(ctx) {
  ...
  if (ctx.state.messages.length > ctx.state.visibleCount) {
    ...
    more.addEventListener('click', async () => {
      ...
      requestAnimationFrame(() => {
        ctx.state.rootEl?.querySelector('#thread-messages-area')?.scrollTo({ top: 0 });
      });
    });
  ...
  if (!visibleMessages.length) {
    ...
    visibleMessages.forEach((message) => {
      list.appendChild(createMessageRow(ctx, message));
    });
  }
...
function createMessageRow(ctx, message) {
  ...
  const avatar = isUser
    ? createAvatar(ctx.getCurrentUserDisplayProfile().avatar, ctx.getCurrentUserDisplayProfile().name || '我', 'sm')
    : createAvatar(ctx.getSpeakerAvatar(message.characterId), ctx.getSpeakerName(message.characterId), 'sm');

  const body = el('div', 'message-body');

  body.appendChild(el('div', 'message-name', isUser ? ctx.getCurrentUserDisplayProfile().name || '我' : ctx.getSpeakerName(message.characterId)));

  if (isUser) {
    body.appendChild(createUserMessageContent(ctx, message));
  } else {
    body.appendChild(createAssistantMessageContent(ctx, message));
  }
...
function createUserMessageContent(ctx, message) {
  const wrap = el('div', 'message-content-card');
  appendMessagePayload(ctx, wrap, message);
  return wrap;
}

function createAssistantMessageContent(ctx, message) {
...
function createInputBar(ctx) {
  ...
  textarea.addEventListener('focus', () => {
    document.documentElement.style.setProperty('--chat-input-focused', '1');
  });

  textarea.addEventListener('blur', () => {
    document.documentElement.style.setProperty('--chat-input-focused', '0');
  });
...
function createQuotePreview(ctx) {
  ...
  quote.append(
    el('div', 'quote-preview-text', `引用 ${ctx.getSpeakerName(message.characterId)}：${ctx.getMessagePreview(message)}`),
    iconButton('close', '取消引用')
  );

  quote.querySelector('button').addEventListener('click', async () => {
