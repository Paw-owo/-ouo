...
function createMessageRow(state, message, pageEl) {
  ...
  row.append(
    createReasoningStack(message, role),
    body
  );
...
function createReasoningStack(message, role) {
  const stack = el('section', `chat-reasoning-stack role-${role}`);
...
function createTransferCard(message) {
  ...
  top.append(
    el('div', 'chat-mini-card-title', message.title || '转账消息'),
...
function createShopCard(message) {
  ...
  const image = String(message.itemImage || message.imageBase64 || message.image || '').trim();
...
function getTargetInfo(state, message) {
  if (message.role === 'user') {
    ...
    return { name: user.name || '我', avatar: user.avatar || user.avatarUrl || user.imageBase64 || '' };
  }
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-message-row.role-system .chat-message-bubble {
    ...
    .chat-message-body.role-system .chat-message-actions {
...
// 改了什么：修消息显示错乱；头像名字一行、气泡稳定对齐、思维链展开完整、操作按钮收紧、图片和表情固定容器。
// 会不会影响其他文件：不会，导出函数不变。
// 更新记忆里该文件的导出函数：无变化。
