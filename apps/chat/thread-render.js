...
function createReasoningStack(message, role, mode = 'bubble') {
  ...
  const card = el('section', 'chat-chain-card');
  card.dataset.open = 'false';

  const toggle = safeButton('chat-chain-toggle', '展开思考过程');
  toggle.setAttribute('aria-expanded', 'false');

  const summary = el('span', 'chat-chain-summary');
  summary.append(
    createLineIcon('thought'),
    el('span', 'chat-chain-summary-text', getReasoningSummary(message))
  );

  const arrow = el('span', 'chat-chain-arrow');
  ...
  const detail = el('div', 'chat-chain-detail');
  const list = el('div', 'chat-chain-list');

  steps.forEach((step, index) => {
    list.appendChild(createChainStep(step, index));
  });
...
function buildReasoningSteps(message) {
  ...
  if (normalizeMultiline(message.thinking)) {
    steps.push({
      type: 'thinking',
      title: '整理想法',
      summary: getReasoningSummary(message),
      detail: normalizeMultiline(message.thinking)
    });
  }
  ...
  normalizeToolCalls(message.memoryWrites || message.memories || message.memoryUpdates).forEach((memory, index) => {
    steps.push({
      type: 'memory',
      title: `写进记忆 ${index + 1}`,
      summary: trimOneLine(memory.content || memory.summary || memory.text || memory, 28) || '我记下了一点重要的小事',
...
function createChainStep(step, index) {
  const item = el('section', `chat-chain-step type-${step.type || 'tool'} status-${step.status || 'done'}`);
  item.dataset.open = 'false';

  const head = safeButton('chat-chain-step-head', step.title || '查看详情');

  const iconWrap = el('span', 'chat-chain-step-icon');
  iconWrap.appendChild(createLineIcon(getStepIcon(step.type)));

  const body = el('span', 'chat-chain-step-main');
  body.append(
    el('span', 'chat-chain-step-title', step.title || `步骤 ${index + 1}`),
    el('span', 'chat-chain-step-summary', step.summary || '已完成')
  );

  const arrow = el('span', 'chat-chain-step-arrow');
  arrow.appendChild(createLineIcon('chevron'));

  head.append(iconWrap, body, arrow);
  head.addEventListener('click', () => {
    item.dataset.open = item.dataset.open === 'true' ? 'false' : 'true';
  });

  const detail = el('div', 'chat-chain-step-detail');
  const pre = el('pre', 'chat-chain-step-pre');
  pre.textContent = step.detail || '这里没有更多内容。';
...
function getReasoningSummary(message) {
  ...
  if (!thinking) return '我正在认真想怎么回应你。';
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-chain-summary svg {
      flex: 0 0 auto;
      color: var(--accent);
    }

    .chat-chain-summary-text {
      min-width: 0;
      color: var(--text-secondary);
      font-size: 13px;
      line-height: 1.45;
    ...
    .chat-chain-card[data-open="true"] .chat-chain-arrow {
      transform: rotate(90deg);
    }
    ...
    .chat-chain-card[data-open="true"] .chat-chain-detail {
    ...
    .chat-chain-step:last-child::before {
      display: none;
    }

    .chat-chain-step-head {
      width: 100%;
      min-height: 42px;
      display: grid;
    ...
    .chat-chain-step-head:active {
      transform: scale(0.98);
    }

    .chat-chain-step-icon {
      width: 30px;
      height: 30px;
      position: relative;
      z-index: 1;
    ...
    .chat-chain-step[data-open="true"] .chat-chain-step-arrow {
      transform: rotate(90deg);
    }

    .chat-chain-step-detail {
      max-height: 0;
      overflow: hidden;
      opacity: 0;
      transition: all 200ms ease;
    ...
    .chat-chain-step[data-open="true"] .chat-chain-step-detail {
      max-height: 280px;
      opacity: 1;
      overflow-y: auto;
      padding-bottom: 10px;
