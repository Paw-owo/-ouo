...
// 修复：跟随全局开关只改 useGlobal 字段，不污染角色自己的配置
// ═══════════════════════════════════════
...
function createApiSection() {
  ...
  section.append(
    ...
    createSelectRow({
      title: '中转站',
      desc: followGlobal ? '跟随全局默认中转站。' : '关闭跟随全局后，才会优先使用这里。',
    ...
    createInputRow({
      title: '模型名称',
      desc: followGlobal ? '正在跟随全局默认模型。' : '可以手填模型名，也可以填中转站里的模型。',
      value: api.model || '',
    ...
    createSliderRow({
      ...
      suffix: '',
      onChange: (value) => updateApiConfig({
        presencePenalty: Number(value) / 100,
        ...(followGlobal ? {} : { useGlobal: false })
      })
    }),
    createSliderRow({
      ...
      suffix: '',
      onChange: (value) => updateApiConfig({
        frequencyPenalty: Number(value) / 100,
        ...(followGlobal ? {} : { useGlobal: false })
      })
    }),
    createNumberRow({
    ...
    createSwitchRow({
      ...
      onChange: async (checked) => updateApiConfig({
        stream: checked,
        ...(followGlobal ? {} : { useGlobal: false })
      })
    })
...
// ───────────────────
// 读取 API 配置：useGlobal 时读全局默认 API
// ───────────────────
...
// 只在角色自己的 apiConfig 上合并 patch，不经过全局合并
...
function createVoiceSection() {
  ...
  section.append(
    createSwitchRow({
      title: '跟随全局语音',
      desc: followGlobal ? '正在用全局语音配置。' : '已经切换到独立语音配置。',
      checked: followGlobal,
      onChange: async (checked) => {
        ...
        if (checked) {
          ...
          await updateChatConfig({ ttsVoice: tts.voice || own.voice || 'alloy', ttsModel: tts.model || own.model || 'tts-1' });
        }
      }
    }),
    createSwitchRow({
    ...
    createInputRow({
      title: '语音服务',
      desc: followGlobal ? '跟随全局语音服务。' : '填写服务名，留空走全局。',
    ...
    createInputRow({
      title: '语音模型',
      desc: followGlobal ? '跟随全局语音模型。' : '填写 TTS 模型名，留空走全局。',
    ...
    createInputRow({
      title: '声音名',
      desc: followGlobal ? '跟随全局声音。' : '填写音色名，留空走全局。',
...
// 只在角色自己的 voiceConfig 上合并 patch，不经过全局合并
...
function createReplySection() {
  ...
  section.append(
    createSelectRow({
      title: '对话样式',
      desc: '气泡更像聊天，对话更像阅读。',
      value: (getData('app_settings') || {}).bubbleMode === 'dialog' ? 'dialog' : 'bubble',
      options: [
...
function createWorldbookSection() {
  ...
  if (state.worldbooks.length > 40) {
    section.append(createHintCard(`还有 ${state.worldbooks.length - 40} 条没显示，太多了先收着。`));
  }
