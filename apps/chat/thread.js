...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-sticker-center-overlay{position:fixed;inset:0;z-index:9999;display:flex;align-items:center;justify-content:center;padding:22px;background:var(--bg-overlay);animation:chatStickerModalIn 180ms ease both}
    ...
    @media(prefers-reduced-motion:reduce){.chat-thread-page,.chat-thread-search-card,.chat-thread-tool-grid,.chat-thread-tool-card,.chat-sticker-grid-wrap,.chat-sticker-center-overlay{animation:none;transition:none}}
  `;

  document.head.appendChild(style);
}

// 改了什么：只修 MCP 面板过滤逻辑，空 MCP 对象不会再显示。
// 会不会影响其他文件：不会，导出函数不变。
// 更新记忆里该文件的导出函数：无变化。
