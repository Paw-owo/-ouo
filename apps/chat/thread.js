...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .chat-thread-tool-card {
      ...
      font: inherit;
      transition: all 200ms ease;
      animation: chatToolCardIn 240ms ease both;
...
// 改了什么：集中补完消息详情页壳子工具入口，快捷回复、文字版语音、表情、MCP请求、接龙、心情记录、清上下文、转账、配置都能点击执行。
// 会不会影响其他文件：会，依赖 thread-actions.js 的发送函数和 thread-ai.js 的 visibleCount 支持；不需要改导出。
// 更新记忆里该文件的导出函数：无变化。
