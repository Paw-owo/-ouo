...
export async function mountChatThread(containerEl, options = {}) {
  ...
  state.toolItemsCache = [];
  state.toolPagesCache = [[]];
...
async function loadOptionalModules() {
  if (!renderThreadMessagesFn) {
    const mod = await importOptionalModule('./thread-render.js', '消息渲染');
  ...
  if (
    ...
    const mod = await importOptionalModule('./thread-actions.js', '消息操作');
  ...
  if (!mountThreadCallFn || !unmountThreadCallFn) {
    const mod = await importOptionalModule('./thread-call.js', '电话');
  ...
  if (!checkThreadProactiveMessagesFn) {
    const mod = await importOptionalModule('./thread-ai.js', 'AI 主动消息');
...
async function importOptionalModule(path, label) {
  try {
    ...
    console.error(`[chat-thread] ${label}模块加载失败：${path}`, error);
    showModuleWarning(`${label}模块没有加载成功，聊天可能只能显示简版。`);
    return null;
  }
}

function showModuleWarning(message) {
  if (moduleWarningShown) return;
  moduleWarningShown = true;
  window.setTimeout(() => {
    showToast(message || '聊天模块没有加载成功');
  }, 120);
}
...
function render() {
  ...
  const previousList = state.rootEl.querySelector?.('#chat-thread-list');
  const previousScrollTop = previousList ? previousList.scrollTop : null;
  ...
  page.dataset.searchQuery = state.searchValue || '';
  ...
  renderMessagesIntoPage(page);

  if (previousScrollTop !== null) {
    requestAnimationFrame(() => {
      const nextList = state.rootEl?.querySelector?.('#chat-thread-list');
      if (nextList) nextList.scrollTop = previousScrollTop;
    });
  }
}

function renderMessagesIntoPage(page) {
  if (!page) return;
...
function refreshMessageListOnly({ keepScroll = true } = {}) {
  if (!state.rootEl || !state.mounted) return;

  const page = state.rootEl.querySelector('.chat-thread-page');
  const oldList = page?.querySelector('#chat-thread-list');
  if (!page || !oldList) {
    render();
    return;
  }

  const previousScrollTop = oldList.scrollTop;
  const area = page.querySelector('.chat-thread-area');
  if (!area) {
    render();
    return;
  }
...
function isInputFocused() {
  ...
  if (!state.rootEl || !(active instanceof Element) || !state.rootEl.contains(active)) {
    return false;
  }
...
function createSearchCard() {
  ...
  input.addEventListener('input', () => {
    ...
    refreshMessageListOnly({ keepScroll: false });
  });
...
async function handleDice() {
  ...
  try {
    ...
    console.error('[chat-thread] dice failed', error);
    showToast('骰子没发出去，界面恢复啦');
  } finally {
    state.isSending = false;
    await reloadAndRender();
  }
...
async function handleRps() {
  ...
  try {
    ...
    console.error('[chat-thread] rps failed', error);
    showToast('猜拳没发出去，界面恢复啦');
  } finally {
    state.isSending = false;
    await reloadAndRender();
  }
...
function applySearchHighlights(page) {
  const query = String(state.searchValue || '').trim();
  if (!query || !page) return;

  const roots = page.querySelectorAll(
    '.chat-thread-fallback-content,.chat-message-text,.chat-message-content,.chat-bubble-content,.chat-dialog-content,.chat-card-title,.chat-card-desc'
  );
...
function highlightTextNode(root, query) {
  ...
  const walker = document.createTreeWalker(root, NodeFilter.SHOW_TEXT, {
    acceptNode(node) {
      ...
      if (node.parentElement?.closest('mark,.chat-search-mark')) {
        return NodeFilter.FILTER_REJECT;
      }
...
function getVisibleMessages() {
  ...
  return list.slice(Math.max(0, list.length - state.visibleCount));
}
...
function pickStickerBatchFiles() {
  ...
  input.addEventListener('change', async () => {
    ...
    if (skipped.length) {
      showToast(`跳过了：${skipped.slice(0, 3).join('、')}${skipped.length > 3 ? ' 等' : ''}`);
    }
...
function openToolSheet() {
  ...
  state.toolPage = 0;
  state.toolItemsCache = getThreadTools();
...
function createToolSheetShell() {
  ...
  viewport.addEventListener('pointercancel', resetToolSwipeStart);
...
function updateToolSheetPage() {
  ...
  const tools = state.toolItemsCache.length ? state.toolItemsCache : getThreadTools();
...
function createInlineIcon(name) {
  ...
  if (name === 'dice') {
    ...
    path('M12 4 3 20h18L12 4Z');
    path('M12 9v4');
    path('M12 16h.01');
  } else if (name === 'message-off') {
    path('M4 5h16v10H8l-4 4V5Z');
    path('M8 9h8');
    path('M8 12h5');
    path('M6 18 18 6');
  } else {
...
function injectStyle() {
  ...
  style.textContent = `
    .chat-thread-page{--chat-keyboard-offset:0px;gap:0;height:calc(100dvh - var(--chat-keyboard-offset,0px));max-height:calc(100dvh - var(--chat-keyboard-offset,0px));overflow:hidden;transition:height 200ms ease,max-height 200ms ease}
    ...
    .chat-load-more-btn{min-height:34px;padding:0 14px;border-radius:999px;background:var(--bg-card);color:var(--text-secondary);box-shadow:var(--shadow-sm);font:inherit;font-size:12px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-search-mark{padding:0 .12em;border-radius:4px;background:var(--accent-light);color:var(--text-primary)}
    ...
    .chat-relationship-lock-action{min-height:34px;padding:0 12px;border-radius:999px;background:var(--accent);color:var(--bubble-user-text);box-shadow:var(--shadow-sm);font:inherit;font-size:12px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-thread-sheet-item{min-height:44px;display:inline-flex;align-items:center;justify-content:center;gap:6px;border-radius:18px;background:var(--bg-card);color:var(--text-primary);box-shadow:var(--shadow-sm);font:inherit;font-size:14px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-thread-tool-card{height:72px;min-height:72px;display:grid;grid-template-rows:30px auto;align-items:center;justify-items:center;gap:5px;padding:8px 6px 7px;border-radius:20px;background:var(--bg-card);color:var(--text-primary);box-shadow:var(--shadow-sm);font:inherit;transition:transform 200ms ease,background 200ms ease,color 200ms ease;animation:chatToolCardIn 220ms ease both;animation-delay:var(--tool-delay,0ms)}
    ...
    .chat-thread-tool-dot{width:7px;height:7px;border-radius:999px;background:var(--text-hint);opacity:.35;transition:width 200ms ease,opacity 200ms ease,background 200ms ease}
    ...
    .chat-mcp-head-btn{width:48px;height:48px;display:inline-flex;align-items:center;justify-content:center;border-radius:18px;background:transparent;color:var(--text-primary);transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-mcp-row{min-height:76px;display:grid;grid-template-columns:46px minmax(0,1fr) auto 64px;align-items:center;gap:14px;padding:0;background:transparent;color:var(--text-primary);font:inherit;text-align:left;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-mcp-toggle{position:relative;width:64px;height:38px;display:inline-flex;align-items:center;padding:4px;border-radius:999px;background:var(--surface-muted);box-shadow:var(--shadow-sm);transition:background 200ms ease}
    .chat-mcp-toggle-dot{width:30px;height:30px;border-radius:999px;background:var(--bg-card);box-shadow:var(--shadow-sm);transition:transform 200ms ease}
    ...
    .chat-sticker-tool-btn{min-height:42px;display:inline-flex;align-items:center;justify-content:center;gap:6px;border-radius:18px;background:var(--bg-card);color:var(--text-secondary);box-shadow:var(--shadow-sm);font:inherit;font-size:12px;transition:transform 200ms ease,background 200ms ease,color 200ms ease;touch-action:manipulation}
    ...
    .chat-sticker-cell{position:relative;aspect-ratio:1;min-width:0;display:flex;align-items:center;justify-content:center;padding:4px;border-radius:18px;background:transparent;color:var(--text-primary);font:inherit;transition:transform 200ms ease,background 200ms ease;touch-action:manipulation}
    ...
    .chat-sticker-modal-pick,.chat-sticker-modal-option{min-height:44px;display:inline-flex;align-items:center;justify-content:center;gap:8px;border-radius:18px;background:var(--surface-muted);color:var(--text-primary);box-shadow:var(--shadow-sm);font:inherit;font-size:14px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-sticker-modal-btn{min-height:44px;border-radius:18px;box-shadow:var(--shadow-sm);font:inherit;font-size:14px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-choice-item{color:var(--text-primary);text-align:left;font:inherit;font-size:14px;line-height:1.5;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
    ...
    .chat-config-switch-ui{position:relative;width:46px;height:28px;border-radius:999px;background:var(--surface-muted);box-shadow:var(--shadow-sm);transition:background 200ms ease}
    .chat-config-switch-ui::after{content:"";position:absolute;top:4px;left:4px;width:20px;height:20px;border-radius:999px;background:var(--bg-card);box-shadow:var(--shadow-sm);transition:transform 200ms ease}
    ...
    .chat-config-btn,.chat-transfer-btn,.chat-clear-btn,.chat-mini-btn{min-height:44px;display:inline-flex;align-items:center;justify-content:center;border-radius:16px;box-shadow:var(--shadow-sm);font:inherit;font-size:14px;transition:transform 200ms ease,background 200ms ease,color 200ms ease}
...
// 改了什么：把你说的 10 个问题都收了，输入法、搜索、滚动、MCP、配置检查、工具提示都修了。
// 会不会影响其他文件：不会，只动 apps/chat/thread.js。
