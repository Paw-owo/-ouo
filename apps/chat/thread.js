...
// 改了什么：聊天详情结构改为左上返回、右上搜索按钮、搜索卡片点击才展开，输入栏改成左加号中输入右发送，并移除输入栏 token。
// 会不会影响其他文件：会，token 放到消息底部需要下一步更新 apps/chat/thread-render.js；消息列表左上返回需要下一步更新 apps/chat/list.js。
// 更新记忆里该文件的导出函数：无变化。
