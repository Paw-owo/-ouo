...
// 改了什么：只把 thread-actions.js 新增导出的 sendCardMessage 接入动态导入层，不改聊天主逻辑。
// 会不会影响其他文件：不会；但要真正发送礼物/商店小卡片，还需要 apps/chat/thread-actions.js 已导出 sendCardMessage，并由商店/钱包写入对应消息结构。
// 更新记忆里该文件的导出函数：无变化。
