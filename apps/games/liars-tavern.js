...
async function getAiDecision(npc) {
  ...
  try {
    ...
    if (parsed.memory && npc.characterId) {
      await writeMemory(npc.characterId, parsed.memory);
    }
...
function buildNpcPrompt(npc) {
  ...
  const memoryRule = npc.characterId
    ? '我是用户已创建的人设，重要事件可以写入 memory。'
    : '我是随机匹配的陌生陪玩，我不能写入 memory，memory 必须为空字符串。';
...
async function writeEventMemories(event) {
  ...
  await Promise.all(unique.map(async player => {
    ...
    if (content) await writeMemory(player.characterId, content);
  }));
}
...
async function writeMemory(characterId, content) {
...
function injectStyle() {
  ...
  style.textContent = `
    ...
    .lt-vignette {
      ...
      pointer-events: none;
      background:
        radial-gradient(ellipse at center, transparent 24%, rgba(0,0,0,0.74) 100%),
        url("data:image/svg+xml,%3Csvg viewBox='0 0 200 200' xmlns='http://www.w3.org/2000/svg'%3E%3Cfilter id='n'%3E%3CfeTurbulence type='fractalNoise' baseFrequency='0.7' numOctaves='3' stitchTiles='stitch'/%3E%3C/filter%3E%3Crect width='100%25' height='100%25' filter='url(%23n)' opacity='0.035'/%3E%3C/svg%3E");
      z-index: 1;
    }
    ...
    .lt-topbar {
      ...
      gap: 12px;
      padding: 10px 0 16px;
      background: linear-gradient(to bottom, rgba(7,3,8,0.94), rgba(7,3,8,0));
    }
    ...
    .lt-title-wrap h1 {
      ...
      font-family: "Playfair Display", "STKaiti", "SimSun", serif;
      font-style: italic;
      font-size: 22px;
      font-weight: 700;
      letter-spacing: 0.14em;
      text-shadow: 0 0 20px rgba(196,154,42,0.34), 0 0 40px rgba(232,25,122,0.16);
    }
    ...
    .lt-icon-btn {
      ...
      box-shadow: 0 2px 12px rgba(0,0,0,0.28), inset 0 0 0 1px var(--lt-gold-dim);
    }
    ...
    .lt-flame-outer {
      fill: var(--lt-pink-neon);
      opacity: 0.82;
      animation: ltFlame 1.8s ease-in-out infinite;
    }

    .lt-flame-inner {
      fill: var(--lt-text-primary);
      opacity: 0.58;
      animation: ltFlame 1.25s ease-in-out infinite reverse;
    }

    .lt-flame-glow {
      fill: var(--lt-pink-neon);
      opacity: 0.08;
      animation: ltGlow 2s ease-in-out infinite;
    }
    ...
    .lt-scene-enter {
      animation: ltDoorOpen 0.78s cubic-bezier(0.4, 0, 0.2, 1) both;
    }
    ...
    .lt-gameover-card {
      position: relative;
      padding: 20px;
      background: rgba(17,10,15,0.88);
      box-shadow:
        0 2px 12px rgba(0,0,0,0.36),
        inset 0 0 40px rgba(0,0,0,0.42),
        inset 0 0 0 1px rgba(122,90,16,0.58);
    }
    ...
    .lt-rule-card::before,
    .lt-table::before,
    .lt-hand-panel::before,
    .lt-log-panel::before,
    .lt-gameover-card::before {
      content: "";
      position: absolute;
      top: 0;
      left: 0;
      width: 18px;
      height: 18px;
      background-image: url("data:image/svg+xml,%3Csvg width='18' height='18' viewBox='0 0 18 18' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M0 18 L0 0 L18 0' stroke='%23c49a2a' stroke-width='1.5' fill='none'/%3E%3Cpath d='M3 15 L3 3 L15 3' stroke='%237a5a10' stroke-width='0.6' fill='none' opacity='0.7'/%3E%3Ccircle cx='1' cy='1' r='1.5' fill='%23c49a2a' opacity='0.9'/%3E%3C/svg%3E");
      pointer-events: none;
    }
    ...
    .lt-gameover-card::after {
      ...
      width: 18px;
      height: 18px;
      background-image: url("data:image/svg+xml,%3Csvg width='18' height='18' viewBox='0 0 18 18' xmlns='http://www.w3.org/2000/svg'%3E%3Cpath d='M18 0 L18 18 L0 18' stroke='%23c49a2a' stroke-width='1.5' fill='none'/%3E%3Cpath d='M15 3 L15 15 L3 15' stroke='%237a5a10' stroke-width='0.6' fill='none' opacity='0.7'/%3E%3Ccircle cx='17' cy='17' r='1.5' fill='%23c49a2a' opacity='0.9'/%3E%3C/svg%3E");
      pointer-events: none;
    }
    ...
    .lt-mask-face {
      fill: var(--lt-bg-deep);
      stroke: var(--lt-gold-worn);
      stroke-width: 1;
    }

    .lt-mask-face.is-user {
      stroke: var(--lt-pink-dim);
    }
    ...
    .lt-mask-glow {
      fill: var(--lt-pink-neon);
      opacity: 0.9;
      animation: ltEye 3s ease-in-out infinite;
    }
    .lt-mask-mouth,
    .lt-mask-line {
      fill: none;
    ...
    .lt-small-btn {
      ...
      box-shadow: inset 0 0 0 1px rgba(122,16,64,0.7);
      font-size: 12px;
      letter-spacing: 0.08em;
    }
    ...
    .lt-character-card {
      ...
      box-shadow: inset 0 0 0 1px rgba(122,90,16,0.34);
    }

    .lt-character-card.is-selected {
      background: rgba(61,8,32,0.72);
      box-shadow: 0 0 18px rgba(232,25,122,0.12), inset 0 0 0 1px rgba(232,25,122,0.46);
    }
    ...
    .lt-avatar {
      ...
      overflow: hidden;
      flex: 0 0 auto;
      background: rgba(7,3,8,0.62);
      box-shadow: inset 0 0 0 1px var(--lt-gold-dim);
    }
    ...
    .lt-empty {
      padding: 18px;
      color: var(--lt-text-dim);
      background: rgba(28,16,24,0.54);
      line-height: 1.7;
      box-shadow: inset 0 0 0 1px rgba(122,90,16,0.32);
    }
    ...
    .lt-main-btn {
      ...
      padding: 12px 14px;
      color: var(--lt-text-primary);
      background: rgba(28,16,24,0.78);
      box-shadow: inset 0 0 0 1px rgba(122,16,64,0.72), 0 2px 12px rgba(0,0,0,0.18);
      font-size: 14px;
    ...
    .lt-gold-btn {
      color: var(--lt-gold-light);
      box-shadow: inset 0 0 0 1px rgba(196,154,42,0.56), 0 0 18px rgba(196,154,42,0.08);
    }

    .lt-main-btn:disabled,
    .lt-hand-card:disabled {
      opacity: 0.42;
      cursor: default;
      transform: none;
    }
    ...
    .lt-table-card {
      ...
      width: 92px;
      height: 122px;
      background: radial-gradient(circle at 50% 0%, rgba(232,25,122,0.16), rgba(17,10,15,0.96) 62%);
      box-shadow:
        0 0 28px rgba(232,25,122,0.14),
    ...
    .lt-table-card strong {
      color: var(--lt-gold-light);
      font-family: "Courier New", monospace;
      font-size: 38px;
      line-height: 1;
      text-shadow: 0 0 20px rgba(232,25,122,0.2);
    }
    ...
    .lt-seat {
      ...
      min-height: 78px;
      padding: 10px;
      background: rgba(28,16,24,0.66);
      box-shadow: inset 0 0 0 1px rgba(122,90,16,0.32);
      transition: all 200ms ease;
    }

    .lt-seat.is-active {
      background: rgba(61,8,32,0.66);
      box-shadow: 0 0 18px rgba(232,25,122,0.14), inset 0 0 0 1px rgba(232,25,122,0.52);
    }
    ...
    .lt-cylinder i {
      width: 6px;
      height: 6px;
      background: rgba(122,90,16,0.32);
      box-shadow: 0 0 0 1px rgba(196,154,42,0.16);
    }

    .lt-cylinder i.is-fired {
      background: rgba(0,0,0,0.72);
    }

    .lt-cylinder i.is-current {
      background: var(--lt-pink-neon);
      box-shadow: 0 0 8px rgba(232,25,122,0.56);
    }
    ...
    .lt-card-back {
      ...
      width: 52px;
      height: 72px;
      background: linear-gradient(145deg, rgba(61,8,32,0.92), rgba(17,10,15,0.98));
      box-shadow: inset 0 0 0 1px rgba(196,154,42,0.48), 0 3px 10px rgba(0,0,0,0.28);
    ...
    .lt-turn-badge {
      padding: 7px 10px;
      color: var(--lt-pink-neon);
      background: rgba(61,8,32,0.5);
      box-shadow: inset 0 0 0 1px rgba(232,25,122,0.3);
      font-size: 12px;
      white-space: nowrap;
    }
    ...
    .lt-hand-card {
      ...
      height: 92px;
      color: var(--lt-text-primary);
      background: rgba(17,10,15,0.94);
      box-shadow:
        inset 0 0 0 1px rgba(196,154,42,0.5),
    ...
    .lt-hand-card.is-picked {
      transform: translateY(-10px);
      box-shadow:
        0 0 18px rgba(232,25,122,0.22),
        inset 0 0 0 1px rgba(232,25,122,0.72),
        inset 0 0 24px rgba(0,0,0,0.45);
    }

    .lt-hand-card.is-joker span {
      color: var(--lt-pink-neon);
      text-shadow: 0 0 12px rgba(232,25,122,0.45);
    }
    ...
    .lt-log-item {
      padding: 10px 12px;
      background: rgba(28,16,24,0.56);
      box-shadow: inset 0 0 0 1px rgba(122,90,16,0.22);
    }
    ...
    .lt-log-item.is-challenge p,
    .lt-log-item.is-caught p { color: var(--lt-pink-neon); }
    .lt-log-item.is-danger p,
    .lt-log-item.is-dead p { color: #d8a1a1; }
    .lt-log-item.is-honest p,
    .lt-log-item.is-survive p { color: var(--lt-text-primary); }
    ...
    .lt-sparks i {
      ...
      height: 3px;
      background: var(--lt-pink-neon);
      box-shadow: 0 0 4px var(--lt-pink-neon), 0 0 8px rgba(232,25,122,0.5);
      opacity: 0;
      animation: ltSpark var(--dur) ease-out infinite;
      animation-delay: var(--delay);
    }

    .lt-shake {
      animation: ltExposeShake 0.55s cubic-bezier(0.36, 0.07, 0.19, 0.97);
    }
    ...
    .lt-shake::after {
      ...
      z-index: 99;
      pointer-events: none;
      background: radial-gradient(circle at center, rgba(232,25,122,0.35), rgba(122,16,64,0.1));
      animation: ltFlashFade 0.6s ease-out forwards;
    }
    ...
    @keyframes ltFlame {
      0%, 100% { transform: translateX(0) scaleY(1); opacity: 0.82; }
      45% { transform: translateX(-0.8px) scaleY(1.16); opacity: 0.68; }
      70% { transform: translateX(0.8px) scaleY(0.9); opacity: 0.95; }
    }
    ...
    @keyframes ltDoorOpen {
      0% { opacity: 0; transform: scale(1.04) translateY(10px); filter: blur(4px); }
      100% { opacity: 1; transform: scale(1) translateY(0); filter: blur(0); }
    }

    @keyframes ltSpark {
      0% { opacity: 0.9; transform: translateY(0) translateX(0) scale(1); }
      50% { opacity: 0.5; transform: translateY(-30px) translateX(var(--drift)) scale(0.6); }
      100% { opacity: 0; transform: translateY(-60px) translateX(calc(var(--drift) * 1.5)) scale(0.2); }
    }

    @keyframes ltExposeShake {
      0% { transform: translate(0,0); filter: brightness(1); }
      10% { transform: translate(-5px, 2px); filter: brightness(1.5) saturate(1.6); }
      20% { transform: translate(5px, -3px); filter: brightness(1); }
      30% { transform: translate(-3px, 4px); }
      40% { transform: translate(4px, -2px); filter: brightness(1.7); }
      50% { transform: translate(-2px, 1px); filter: brightness(1); }
      60% { transform: translate(2px, 2px); }
      100% { transform: translate(0,0); filter: brightness(1); }
    }

    @keyframes ltFlashFade {
      0% { opacity: 1; }
      60% { opacity: 0.4; }
      100% { opacity: 0; }
    }
