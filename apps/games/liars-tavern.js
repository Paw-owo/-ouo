...
export async function mount(container, mountOptions = {}) {
  ...
  if (!state.players.length || shouldRefreshPlayersForSettings()) {
...
function normalizeSettings(raw) {
  ...
  return {
    ...
    selectedCharacterIds: Array.isArray(source.selectedCharacterIds) ? source.selectedCharacterIds : []
  };
}
...
function normalizeState(raw) {
  ...
  return {
    ...
    tavern: {
      ...
      bubblePlayerId: tavern.bubblePlayerId || '',
      bubbleText: tavern.bubbleText || '',
...
function shouldRefreshPlayersForSettings() {
  if (!state) return false;

  const selectedIds = settings.selectedCharacterIds.filter((id) => characters.some((character) => character.id === id));
  const currentCharacterIds = state.players
    .filter((player) => player.kind === 'character')
    .map((player) => player.id);

  if (!selectedIds.length) return false;
  if (selectedIds.length !== currentCharacterIds.length) return true;

  return selectedIds.some((id) => !currentCharacterIds.includes(id));
}

function createPlayers() {
  const selected = getSelectedCharacters();
  const picked = selected.slice(0, 5).map((character, index) => ({
    id: character.id || `character-${index}`,
    name: character.name || `旅人 ${index + 1}`,
    avatar: character.avatar || '',
    kind: 'character',
    chips: 6,
    suspicion: 28 + index * 8
  }));

  const fallback = NPC_NAMES.slice(0, Math.max(0, 5 - picked.length)).map((name, index) => ({
    id: `npc-${index + 1}`,
    name,
    avatar: '',
    kind: 'npc',
...
function getSelectedCharacters() {
  const selectedIds = new Set(settings.selectedCharacterIds);

  if (selectedIds.size) {
    return characters.filter((item) => selectedIds.has(item.id));
  }
...
function renderSeats() {
  ...
  state.players.forEach((player, index) => {
    ...
    seat.innerHTML = `
      <div class="tavern-speech ${player.id === state.tavern.bubblePlayerId && state.tavern.bubbleText ? 'show' : ''}">${escapeHtml(state.tavern.bubbleText || '')}</div>
...
async function startNextRound() {
  ...
  state.tavern.revealed = false;
  state.tavern.bubblePlayerId = '';
  state.tavern.bubbleText = '';
...
function npcAutoStep(reason) {
  ...
  setBubble(npc.id, `${nextCount} 张 ${nextRank}`);
}
...
async function askDealer(context) {
  ...
  try {
    ...
    setBubble(getBubblePlayerIdFromText(finalText), finalText.slice(0, 28));
    await recordToChat(finalText);
  } catch (_) {
    if (!mounted || !state) return;
    const fallback = fallbackDealerText();
    replaceLastAssistant(fallback);
    setBubble(getBubblePlayerIdFromText(fallback), fallback.slice(0, 28));
  } finally {
    if (!mounted || !state) return;
    thinking = false;
    saveState();
...
function getBubblePlayerIdFromText(text) {
  if (!state) return 'user';

  const content = String(text || '');
  const namedPlayer = state.players.find((player) => player.id !== 'user' && player.name && content.includes(player.name));

  if (namedPlayer) return namedPlayer.id;

  return state.tavern.spotlightId && state.tavern.spotlightId !== 'user'
    ? state.tavern.spotlightId
    : pickRandomOpponent()?.id || 'user';
}
...
function openMenuSheet() {
  ...
  sheet.innerHTML = `
    ...
    <div class="sheet-description">拉 AI 好友入座，或让酒馆随机组一桌。</div>

    <div class="tavern-menu-actions">
      <button class="btn-ghost" data-action="players"></button>
      <button class="btn-ghost" data-action="random"></button>
    </div>
  ...
  const playersBtn = sheet.querySelector('[data-action="players"]');
  playersBtn.append(localIcon('user-plus', 17), document.createTextNode('拉人组局'));

  const randomBtn = sheet.querySelector('[data-action="random"]');
  randomBtn.append(localIcon('shuffle', 17), document.createTextNode('随机组局'));
  ...
  randomBtn.addEventListener('click', async () => {
    ...
    shuffle(shuffled);
    settings.selectedCharacterIds = shuffled.slice(0, 4).map((item) => item.id);
    saveSettings();

    state.players = createPlayers();
    state.round = 1;
    resetRoundOnly();
    appendLog('system', '酒馆随机拉了一桌人。');
    saveState();
    hideBottomSheet();
    render();
    showToast('随机组局好了');
...
function openPlayersSheet() {
  ...
  sheet.className = 'tavern-sheet';
  sheet.innerHTML = `
    <div class="sheet-title">拉 AI 好友入座</div>
    <div class="sheet-description">从角色管理里创建的人设会出现在这里。最多带 5 位一起玩。</div>
    <div class="tavern-player-picker"></div>
    <button class="btn-primary" data-action="save">保存组局</button>
  `;

  const picker = sheet.querySelector('.tavern-player-picker');
  const selected = new Set(settings.selectedCharacterIds);

  if (!characters.length) {
    const empty = document.createElement('div');
    empty.className = 'tavern-picker-empty';
    empty.textContent = '还没有 AI 人设。可以先去角色管理里创建。';
    picker.appendChild(empty);
  } else {
    characters.forEach((character) => {
      const item = document.createElement('button');
      item.type = 'button';
      item.className = `tavern-picker-item ${selected.has(character.id) ? 'active' : ''}`;

      item.innerHTML = `
        <span class="tavern-picker-avatar"></span>
        <span class="tavern-picker-copy">
          <strong>${escapeHtml(character.name || '未命名')}</strong>
          <small>${escapeHtml(character.description || character.persona || character.systemPrompt || '适合坐上牌桌。')}</small>
        </span>
      `;

      const avatar = item.querySelector('.tavern-picker-avatar');
      if (character.avatar) {
        const img = document.createElement('img');
        ...
        avatar.appendChild(createPlayerGlyph('character'));
      }
  ...
  sheet.querySelector('[data-action="save"]').addEventListener('click', () => {
    settings.selectedCharacterIds = [...selected];
    saveSettings();

    state.players = createPlayers();
    state.round = 1;
    resetRoundOnly();
    appendLog('system', '牌桌重新安排了座位。');
    saveState();
...
function resetRoundOnly() {
  ...
  state.tavern.lastClaimById = '';
  state.tavern.pot = 1;
  state.tavern.trust = 52;
  state.tavern.heat = 18;
  state.tavern.revealed = false;
  state.tavern.bubblePlayerId = '';
  state.tavern.bubbleText = '';
  state.tavern.lastAction = '新座位已经排好，牌局重新开始。';
  state.tavern.whisper = '熟悉或陌生的人设围坐下来，牌桌忽然有了新的气味。';
  state.logs = [];
  dealRound();
}

function setBubble(playerId, text) {
  if (!state || !playerId || !text) return;

  clearBubbleTimer();

  state.tavern.bubblePlayerId = playerId;
  state.tavern.bubbleText = String(text).slice(0, 34);
...
function localIcon(name, size = 18) {
  ...
  if (name === 'back') svg.append(svgEl('path', { d: 'M15 18l-6-6 6-6' }));
  else if (name === 'settings') svg.append(svgEl('circle', { cx: '12', cy: '12', r: '3' }), svgEl('path', { d: 'M19 12a7 7 0 0 0-.1-1.2l2-1.5-2-3.4-2.4 1a7.5 7.5 0 0 0-2-1.1L12 3H8l-.5 2.8a7.5 7.5 0 0 0-2 1.1l-2.4-1-2 3.4 2 1.5A7 7 0 0 0 3 12c0 .4 0 .8.1 1.2l-2 1.5 2 3.4 2.4-1a7.5 7.5 0 0 0 2 1.1L8 21h4l.5-2.8a7.5 7.5 0 0 0 2-1.1l2.4 1 2-3.4-2-1.5c.1-.4.1-.8.1-1.2z' }));
  else if (name === 'send') svg.append(svgEl('path', { d: 'M4 12l16-8-5 16-3-6-8-2zM12 14l3-4' }));
  else if (name === 'cards') svg.append(svgEl('rect', { x: '5', y: '4', width: '10', height: '15', rx: '3', transform: 'rotate(-8 10 11.5)' }), svgEl('rect', { x: '9', y: '5', width: '10', height: '15', rx: '3', transform: 'rotate(7 14 12.5)' }));
  else if (name === 'check') svg.append(svgEl('path', { d: 'M5 12l4 4L19 6' }));
  else if (name === 'search') svg.append(svgEl('circle', { cx: '10', cy: '10', r: '5' }), svgEl('path', { d: 'M14 14l5 5' }));
  else if (name === 'refresh') svg.append(svgEl('path', { d: 'M18 8a7 7 0 1 0 1 6' }), svgEl('path', { d: 'M18 4v4h-4' }));
  else if (name === 'user-plus') svg.append(svgEl('path', { d: 'M10 12c3 0 5-2 5-5s-2-5-5-5-5 2-5 5 2 5 5 5z' }), svgEl('path', { d: 'M2 22c1-5 4-8 8-8s7 3 8 8' }), svgEl('path', { d: 'M19 8v6M16 11h6' }));
  else if (name === 'shuffle') svg.append(svgEl('path', { d: 'M4 7h3c3 0 4 10 7 10h6' }), svgEl('path', { d: 'M17 14l3 3-3 3' }), svgEl('path', { d: 'M4 17h3c1 0 2-.8 3-2' }), svgEl('path', { d: 'M14 7h6' }), svgEl('path', { d: 'M17 4l3 3-3 3' }));
  else svg.append(svgEl('circle', { cx: '12', cy: '12', r: '8' }));
...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .tavern-player-picker::-webkit-scrollbar {
      display: none;
    }
