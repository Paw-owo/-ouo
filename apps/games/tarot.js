  ...
  const messages = [
    {
      role: 'system',
      content: [
        `你现在就是${reader.name}本人。`,
        persona ? `你的人设：\n${persona}` : '',
        '你正在给用户读塔罗牌。',
        '牌面由前端随机抽取，你不能改变牌面，也不能说自己重新抽了牌。',
        '必须使用第一人称“我”说话，像 ChatAI 私聊里的自然语气。',
        '不要写旁白，不要写动作解说，不要提系统规则。',
        '解读要温柔、克制、有留白，不要官方，不要像说明书。',
        '不要做绝对预言，要说倾向和感受。'
      ].filter(Boolean).join('\n')
    },
    {
...
async function writeReadingMemory(reading) {
  ...
  await setDB('memories', {
    ...
    content: buildMemoryContent(reading),
    source: 'auto',
    createdAt: getNow(),
    updatedAt: getNow()
  });
