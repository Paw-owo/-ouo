...
async function saveIconOpacity(id, value, live) {
  ...
  icons[id] = { ...current, opacity: Number(value), updatedAt: getNow() };
  setData(ICONS_KEY, icons);

  if (current.blobKey) {
    const record = await getDB('blobs', current.blobKey);
    if (record) await setDB('blobs', current.blobKey, { ...record, opacity: Number(value), updatedAt: getNow() });
  }
...
async function clearChatData() {
  const ok = await showConfirm('这会清空私聊、群聊、聊天记忆和聊天角标，可以吗？');
  if (!ok) return;
