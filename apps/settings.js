...
function render(nextRoute = route) {
  ...
  rootEl.innerHTML = '';
  rootEl.className = 'settings-app-shell';
...
function renderThemePage() {
  ...
  mode.append(actionRow([
    actionBtn('star', '浅色', () => {
      setThemeMode('light');
      showToast('浅色模式换好啦 ๑˃ ᵕ ˂๑');
      emitRefresh();
      render('theme');
    }),
    actionBtn('eye', '夜间', () => {
      setThemeMode('dark');
      showToast('夜间模式换好啦 ՞⩌⌯⩌՞ ᶻ 𝗓');
      emitRefresh();
      render('theme');
    })
  ]));
...
function renderMcpPage() {
  ...
  settings.mcpServers.forEach((server) => {
    ...
    item.append(actionRow([
      actionBtn(server.enabled ? 'stop' : 'play', server.enabled ? '停用' : '启用', () => toggleMcp(server.id)),
...
function renderCloudPage() {
  ...
  const enabled = switchRow('启用云服务', cloud.enabled, (value, row) => {
    ...
    showToast(value ? '云服务开启啦 ˶>ᗜ<˶' : '云服务关好啦 ฅ-u-ฅ');
    render('cloud');
  });
  ...
  form.append(actionRow([
    actionBtn('check', '测试连接', async () => {
      ...
      setData(CLOUD_KEY, next);
      showToast(ok ? '连上啦，云朵小仓库在线 ˶╹ꇴ╹˶' : '没连上，检查地址或密钥哦 •︡ᯅ•︠');
      render('cloud');
    }),
...
function renderDesktopPage() {
  ...
  wallpaper.append(actionRow([
    actionBtn('upload', '上传壁纸', () => uploadBlobImage(WALLPAPER_KEY, WALLPAPER_OPACITY_KEY, '壁纸换好啦 >ᴗoಣ')),
...
function renderIconsPage() {
  ...
  APP_LIST.forEach(([id, name]) => {
    ...
    list.append(listAction(hidden.has(id) ? 'eye-off' : 'star', custom.name || name, hidden.has(id) ? '已隐藏' : custom.image ? '已换图' : '默认图标', [
      actionBtn('edit', '改名', () => renameIcon(id, name)),
      actionBtn('upload', '换图', () => uploadIcon(id)),
      actionBtn(hidden.has(id) ? 'eye' : 'eye-off', hidden.has(id) ? '恢复' : '隐藏', () => toggleIconHidden(id))
    ]));
  });
...
function renderDataPage() {
  ...
  clean.append(actionRow([
    actionBtn('clear', '清聊天记忆', clearChatData),
...
async function loadApiModels(id) {
  ...
  const settings = getSettings();
  settings.apiEndpoints = settings.apiEndpoints.map((api) => api.id === id ? {
    ...api,
    modelList: models,
...
async function testApi(id) {
  const models = await fetchModels(id);
  showToast(models.length ? '连接成功啦 ˵＞𖥦＜˵' : '连接失败了 •︡ᯅ•︠');
}
...
async function fetchTtsModels() {
  ...
  try {
    ...
    if (!base) {
      showToast('先填 TTS 地址哦 ๑ᵒᯅᵒ๑');
      return;
    }
    const res = await fetch(`${base}/v1/models`, {
      headers: tts.apiKey ? { Authorization: `Bearer ${tts.apiKey}` } : {}
    });
    ...
    saveSettings(settings);
    showToast('TTS 模型拉好啦 ˶>ᗜ<˶');
    render('tts');
  } catch {
    showToast('TTS 模型拉取失败 •︡ᯅ•︠');
  }
}
...
function makeButton(className, text, icon, onClick) {
  ...
  btn.type = 'button';
  if (icon) btn.append(createIcon(icon, 17));
...
function listAction(icon, title, desc, buttons) {
  ...
  const mark = el('span', 'settings-row-icon');
  mark.append(createIcon(icon, 18));
