...
function renderWidgetsPage() {
  ...
  WIDGET_BG_LIST.forEach(([key, name]) => {
    ...
    const opacity = Number(record.opacity ?? 100);
    const box = el('div', 'settings-widget-bg-block');

    box.append(listAction('image', name, record.value ? '已换背景' : '还没换背景', [
      actionBtn('upload', '上传', () => uploadWidgetBg(key)),
      actionBtn('delete', '清除', () => clearWidgetBg(key))
    ], imagePreview(record.value || '', name, 'image')));
...
function renderIconsPage() {
  ...
  APP_LIST.forEach(([id, name]) => {
    ...
    box.append(listAction(hidden.has(id) ? 'settings' : 'star', custom.name || name, hidden.has(id) ? '已隐藏' : image ? '已换图' : '默认图标', [
...
function openTtsEditor() {
  ...
  renderEditorModels();
  sheet.body.append(provider.wrap, endpoint.wrap, apiKey.wrap, voice.wrap, model.wrap, modelArea);
...
async function saveIconOpacity(id, value, live) {
  ...
  if (current.blobKey) {
    let record = await getDB('blobs', current.blobKey);
    if (!record) {
      await setDB('blobs', current.blobKey, { key: current.blobKey, value: '', source: '', opacity, updatedAt: getNow() });
    } else {
      await setDB('blobs', current.blobKey, { ...record, opacity, updatedAt: getNow() });
    }
  }
...
async function clearImageDressData() {
  ...
  for (const iconId of Object.keys(iconRecords)) {
    try {
      await deleteDB('blobs', `app_icon_${iconId}`);
    } catch {}
  }
