...
function render() {
  ...
  titleWrap.append(
    el('div', 'nav-title', '设置'),
    el('div', 'nav-subtitle', '轻量配置与本地数据')
  );

  nav.append(backButton, titleWrap);

  const content = el('div', 'content-area');
  const home = renderSettingsHome(settings);
...
function renderSettingsHome(settings) {
  ...
  home.append(
    createSettingsGroup('使用设置', [
      {
        ...
        title: '颜色模式',
        value: getThemeLabel(),
        action: () => openSettingsPanel('主题', renderThemeSection(), '切换预设主题，或微调聊天气泡外观。')
      },
      {
        icon: 'settings',
        title: '偏好设置',
        value: settings.bubbleMode === 'dialog' ? '对话' : '气泡',
        action: () => openSettingsPanel('消息偏好', createMessagePersonal(settings), '调整聊天显示方式和字体大小。')
      },
      {
        icon: 'heart',
        title: '助手',
        value: settings.user.name || '我的资料',
        action: () => openSettingsPanel('我的资料', createProfilePersonal(settings), '设置你的昵称、头像与头像透明度。')
      }
    ]),
    createSettingsGroup('模型与服务', [
      {
        icon: 'heart',
        title: '默认模型',
        value: settings.defaultModel || '未选择',
        action: () => openSettingsPanel('API 配置', renderApiSection(getSettings()), '管理端点、拉取模型并设置默认模型。')
      },
      {
        icon: 'settings',
        title: '供应商',
        value: `${settings.apiEndpoints.length} 个端点`,
        action: () => openSettingsPanel('API 端点', renderApiSection(getSettings()), '新增、编辑、测试或删除 API 端点。')
      },
      {
        icon: 'play',
        title: '语音服务',
        value: settings.ttsGlobal.model || 'tts-1',
        action: () => openSettingsPanel('TTS 配置', renderTtsSection(getSettings()), '配置语音服务、拉取 TTS 模型并测试播放。')
      },
      {
        icon: 'mcp',
        title: 'MCP',
        value: getMcpMeta(settings),
        action: () => openSettingsPanel('MCP 配置', renderMcpSection(getSettings()), '管理可在聊天中使用的 MCP 工具服务器。')
      },
      {
        icon: 'memory',
        title: '世界书',
        value: '后续接入',
        action: () => showToast('后续会接入')
      },
      {
        icon: 'edit',
        title: '快捷短语',
    ...
    createSettingsGroup('个性化', [
      {
        icon: 'expand',
        title: '桌面布局',
        value: getScaleMeta(),
        action: () => openSettingsPanel('桌面布局', renderDesktopLayoutSection(), '调整桌面图标、小组件和 Dock 的显示大小。')
      },
      {
        icon: 'image',
        title: '桌面壁纸',
        value: '图片与透明度',
        action: () => openSettingsPanel('桌面壁纸', renderDesktopWallpaperSection(getSettings()), '设置桌面壁纸和小组件显示开关。')
      },
      {
        icon: 'expand',
        title: '小组件背景',
        value: '区域与单项',
        action: () => openSettingsPanel('小组件背景', renderWidgetBackgroundSection(), '给小组件区域或单个小组件设置背景图。')
      },
      {
        icon: 'smile',
        title: '应用图标',
        value: getHiddenIcons().length ? `${getHiddenIcons().length} 个隐藏` : '全部显示',
        action: () => openSettingsPanel('应用图标', createIconPersonal(), '修改应用名称、图标图片、透明度，也可恢复隐藏图标。')
      },
      {
        icon: 'heart',
        title: '我的资料',
        value: settings.user.name || '未填写',
        action: () => openSettingsPanel('我的资料', createProfilePersonal(getSettings()), '设置你的昵称、头像和头像透明度。')
      }
    ]),
    createSettingsGroup('数据设置', [
      {
        ...
        title: '数据备份',
        value: '导入导出',
        action: () => openSettingsPanel('数据备份', renderDataSection(), '导出完整备份，或导入之前保存的数据。')
      },
      {
        icon: 'memory',
        title: '聊天记录存储',
        value: '本地浏览器',
        action: () => openSettingsPanel('聊天记录存储', renderDataSection(), '查看存储用量，清理聊天记录和记忆。')
      },
      {
        icon: 'clear',
        title: '清理数据',
        value: '谨慎操作',
        action: () => openSettingsPanel('清理数据', renderDataSection(), '清理聊天、记忆或全部本地数据。')
      }
    ]),
    createSettingsGroup('关于', [
      {
        ...
        title: '当前版本',
        value: '静态版',
        action: () => showToast('当前为 GitHub Pages 静态版')
      },
      {
        icon: 'download',
        title: '存储说明',
        value: '本地保存',
        action: () => showToast('配置存 localStorage，大数据存 IndexedDB')
      }
    ])
  );
...
function createSettingsGroup(title, rows) {
  ...
  rows.forEach((row) => {
    card.appendChild(createSettingsRow(row));
  });
...
function openSettingsPanel(title, bodyEl, desc = '') {
  ...
  if (desc) {
    box.appendChild(el('div', 'sheet-description', desc));
  }
...
function createApiEndpointCard(endpoint, settings) {
  ...
  pullButton.addEventListener('click', async () => {
    await withWorking(`models:${endpoint.id}`, pullButton, async () => {
      ...
      hideBottomSheet();
      openSettingsPanel('API 配置', renderApiSection(getSettings()), '管理端点、拉取模型并设置默认模型。');
    });
  });
...
function createMcpCard(server) {
  ...
  const enabledSwitch = switchButton(Boolean(server.enabled), (active) => {
    ...
    hideBottomSheet();
    openSettingsPanel('MCP 配置', renderMcpSection(getSettings()), '管理可在聊天中使用的 MCP 工具服务器。');
  });
...
function renderPersonalSection(settings) {
  ...
  box.append(
    renderDesktopLayoutSection(),
    renderDesktopWallpaperSection(settings),
    renderWidgetBackgroundSection(),
    createIconPersonal(),
...
function renderDesktopLayoutSection() {
  ...
  const iconRange = createScaleRange(scale.iconScale, (value) => saveDesktopScale({ iconScale: value }));
  const widgetRange = createScaleRange(scale.widgetScale, (value) => saveDesktopScale({ widgetScale: value }));
  const dockRange = createScaleRange(scale.dockScale, (value) => saveDesktopScale({ dockScale: value }));

  box.appendChild(formCard([
    field('桌面图标大小', iconRange),
    field('小组件大小', widgetRange),
    field('Dock 大小', dockRange)
  ]));

  box.appendChild(createSoftNote('安卓浏览器如果显示偏大，可以调到图标 82%、小组件 78%、Dock 82%。'));
...
function createDesktopPersonal(settings) {
  const box = el('div', 'settings-panel');
  box.append(renderDesktopLayoutSection(), renderDesktopWallpaperSection(settings), renderWidgetBackgroundSection());
  return box;
}
...
function createHiddenIconsRestoreBlock() {
  ...
  hidden.forEach((appId) => {
    ...
    restoreButton.addEventListener('click', () => {
      ...
      showToast('图标已恢复');
      hideBottomSheet();
      openSettingsPanel('应用图标', createIconPersonal(), '修改应用名称、图标图片、透明度，也可恢复隐藏图标。');
      render();
    });
...
function getThemeLabel() {
  ...
  if (bg.includes('1a1a1a')) return '深色';
  return '浅色';
}
...
function injectSettingsStyle() {
  ...
  style.textContent = `
    ...
    .settings-row + .settings-row {
      box-shadow: inset 0 1px 0 color-mix(in srgb, var(--bg-secondary) 55%, transparent);
    }
