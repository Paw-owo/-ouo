...
function renderSettingsHome(settings) {
  ...
  home.append(
    createSettingsGroup('使用设置', [
      rowConfig('star', '颜色模式', getThemeLabel(), () => openSettingsPanel('颜色模式', renderThemeSection(), '换一套舒服的颜色。')),
    ...
    createSettingsGroup('模型与服务', [
      rowConfig('heart', '模型小管家', settings.defaultModel || '未选择', () => openSettingsPanel('模型小管家', renderApiSection(getSettings()), '管理端点，拉取模型，再选一个默认模型。')),
      rowConfig('settings', '供应商', `${settings.apiEndpoints.length} 个端点`, () => openSettingsPanel('供应商', renderApiSection(getSettings()), '新增、编辑、测试或删除 API 端点。')),
      ...
      rowConfig('edit', '快捷短语', '后续接入', () => showToast('后续会接入')),
      rowConfig('settings', '网络代理', getNetworkProxyMeta(), () => openSettingsPanel('网络代理', renderNetworkProxySection(), '这里只保存代理配置；浏览器静态网页不能真正接管系统代理。'))
    ]),
    createSettingsGroup('个性化', [
      rowConfig('expand', '桌面大小', getScaleMeta(), () => openSettingsPanel('桌面大小', renderDesktopLayoutSection(), '安卓看起来太大时，可以把它们稍微调小一点点。')),
...
function createApiEndpointCard(endpoint, settings) {
  ...
  pullButton.addEventListener('click', async () => {
    await withWorking(`models:${endpoint.id}`, pullButton, async () => {
      ...
      reopenPanel('模型小管家', renderApiSection(getSettings()), '管理端点，拉取模型，再选一个默认模型。');
    });
  });
  ...
  fontInput.addEventListener('change', async () => {
    ...
    await withWorking('custom-font', uploadButton, async () => {
      try {
        ...
        showToast('字体换好啦');
        reopenPanel('字体与显示', renderFontDisplaySection(getSettings()), '把小手机调成你最舒服的样子。');
      } catch (_) {
        showToast('字体好像没换成功');
      } finally {
        fontInput.value = '';
...
function renderPersonalSection(settings) {
  const box = el('div', 'settings-panel');
  box.append(renderDesktopLayoutSection(), renderDesktopWallpaperSection(settings), renderWidgetBackgroundSection(), createIconPersonal(), createProfilePersonal(settings));
  return box;
}
...
function createIconPersonal() {
  ...
  DESKTOP_APPS.forEach((app) => {
    ...
    opacity.addEventListener('change', async () => {
      ...
      const nextImage = getIconImageValue(nextIcons[app.id] || {});
    ...
    fileInput.addEventListener('change', async () => {
      ...
      await withWorking(`icon:${app.id}`, uploadButton, async () => {
        try {
          const source = await imageFileToDataUrl(file, 240, 0.92);
...
function getIconImageValue(iconData) {
  ...
  return iconData.image || iconData.iconImage || iconData.backgroundImage || iconData.imageBase64 || iconData.value || iconData.data || iconData.src || '';
}

function normalizeIconRecord(record) {
  const image = getIconImageValue(record);
  const source = record.imageSource || record.source || image || '';
  ...
  return {
    ...
    imageBase64: image,
    imageSource: source,
    opacity: Math.max(20, Math.min(100, Number(record.opacity) || 100))
  };
}
...
async function updateIconOpacity(app, name, opacity) {
  ...
  const source = current.imageSource || current.source || getIconImageValue(current);
  ...
  const image = await applyImageOpacity(source, opacity / 100);
...
function injectSettingsStyle() {
  ...
  style.textContent = `
    ...
    .settings-app .segmented .segmented-option {
      transition: all 160ms ease;
    }

    .settings-app .segmented .segmented-option.is-pressed {
      transform: scale(0.96);
    }

    .settings-app .segmented .segmented-option:active {
      transform: scale(0.96);
    }
