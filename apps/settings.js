...
function render() {
  ...
  const title = el('div', 'nav-title', '设置');
  const subtitle = el('div', 'nav-subtitle', '所有配置只保存在当前浏览器');
...
function createSection(id, titleText, metaText, bodyEl) {
  ...
  textWrap.append(el('div', 'section-title', titleText), el('div', 'section-meta', metaText));
...
function renderApiSection(settings) {
  ...
  const helper = createSoftNote('新增端点时可以先拉取模型，再从列表里选择，不必手动填写模型名。');
  ...
  box.append(helper, wrapActions(addButton), list);
  return box;
}
...
function createApiEndpointCard(endpoint, settings) {
  ...
  const left = el('div', 'settings-item-main');
  ...
  const mark = el('button', settings.defaultApiEndpointId === endpoint.id ? 'btn-primary subtle' : 'btn-ghost', settings.defaultApiEndpointId === endpoint.id ? '默认' : '设为默认');
  mark.type = 'button';
  ...
  controls.append(pullButton, testButton, editButton, deleteButton);
...
function openApiSheet(endpoint = null) {
  ...
  const form = sheetBase(isEdit ? '编辑 API 端点' : '新增 API 端点', '填写地址和 Key 后，可以先拉取模型，再从列表里选择。');
  ...
  pullButton.addEventListener('click', async () => {
    ...
    await withWorking(`sheet-models:${tempId}`, pullButton, async () => {
...
function renderTtsSection(settings) {
  const box = el('div', 'settings-panel');
  const helper = createSoftNote('TTS 可以拉取模型并选择。朗读真正使用该模型，还需要 core/tts.js 读取 ttsGlobal.model。');
  ...
  pullButton.addEventListener('click', async () => {
    await withWorking('tts-models', pullButton, async () => {
      ...
      after.ttsGlobal = {
        ...after.ttsGlobal,
        provider: after.ttsGlobal.provider || 'openai',
        apiKey: keyInput.value.trim(),
        endpoint: endpointInput.value.trim(),
...
function renderThemeSection() {
  ...
  [
    ...
    item.append(
      el('span', 'theme-card-title', preset.name),
      el('span', 'theme-card-desc', preset.desc)
    );
...
function createDesktopPersonal(settings) {
  ...
  const preview = el('div', 'wallpaper-preview', '暂无壁纸');
  loadWallpaperPreview(preview);

  const uploadButton = button('上传壁纸', 'ghost', 'upload');
  uploadButton.addEventListener('click', () => wallpaperInput.click());

  const opacity = input('', getData(WALLPAPER_OPACITY_KEY) ?? 100, 'range');
  opacity.min = '15';
  opacity.max = '100';
  ...
  opacity.addEventListener('change', async () => {
    await updateWallpaperOpacity(Number(opacity.value));
    emitDesktopRefresh();
    showToast('壁纸透明度已保存');
    render();
  });
  ...
  wallpaperInput.addEventListener('change', async () => {
    ...
    await withWorking('wallpaper', uploadButton, async () => {
      try {
        const source = await imageFileToDataUrl(file, 1920, 0.88);
        ...
        setData(WALLPAPER_OPACITY_KEY, Number(opacity.value));
  ...
  const clearButton = button('清除壁纸', 'ghost', 'clear');
  ...
  details.append(
    preview,
    formCard([
      customRow('壁纸', wrapActions(uploadButton, clearButton, wallpaperInput)),
      field('壁纸透明度', wrapInline(opacity, opacityValue)),
...
async function updateWallpaperOpacity(opacity) {
  const record = await getDB('blobs', WALLPAPER_KEY);
...
1. index.html
指令：
请只改 index.html 一个完整文件。把 APPS 数组里 settings 的 ready:false 改成 ready:true。
保持现有桌面逻辑不变。确认壁纸仍从 IndexedDB blobs/app_wallpaper 的 value 字段读取；这个 value 已经是 settings.js 处理过透明度后的图片，不需要再处理 opacity。
最后自检：设置图标能打开 apps/settings.js，修改壁纸/图标后 desktop:refresh 能刷新桌面。

2. core/tts.js
指令：
请只改 core/tts.js 一个完整文件。resolveConfig(configOverride) 里新增 model 字段，优先级为 configOverride.model > settings.ttsGlobal.model > 'tts-1'。
请求 /v1/audio/speech 的 body 里不要再写死 model:'tts-1'，改为 model: config.model || 'tts-1'。
保持原来的 endpoint/apiKey/voice/provider 逻辑、stopAll、超时、错误提示都不变。
最后自检：settings.js 保存的 ttsGlobal.model 能真正用于 TTS 请求。

3. core/theme.js 或 style.css 暂时不用改
settings.js 已经通过 applyTheme({'font-size-base': 'xxpx'}) 和 saveTheme() 保存字体大小。
*/
