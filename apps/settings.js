...
function openCustomWidgetSheet(widget = null) {
  ...
  let imageSource = widget?.imageSource || '';
  ...
  imageInput.addEventListener('change', async () => {
    ...
    try {
      imageSource = await imageFileToDataUrl(file, 800, 0.9);
      image = await applyImageOpacity(imageSource, Number(opacity.value) / 100);
      preview.innerHTML = '';
      preview.style.backgroundImage = `url("${image}")`;
      showToast('背景图放好啦');
    } catch (_) {
...
function normalizeCustomWidget(widget) {
  ...
  return {
    ...
    imageSource: typeof raw.imageSource === 'string' ? raw.imageSource : '',
...
function injectSettingsStyle() {
  ...
  style.textContent = `
    ...
    .custom-widget-preview.circle,
    .custom-widget-mini.circle {
      border-radius: 999px;
    }

    .custom-widget-preview.rectangle {
      width: 190px;
      height: 104px;
    }
