...
//   from '../core/storage-manager.js': testCloudConnection, uploadSnapshotToCloud, downloadSnapshotFromCloud
...
import { testCloudConnection, uploadSnapshotToCloud, downloadSnapshotFromCloud } from '../core/storage-manager.js';
...
function renderHome() {
  ...
  wrap.append(
    ...
    group('模型与服务', [
      navItem('settings', 'API 小管家', '模型接口、Key、温度和 top_p', 'api'),
...
function renderApiPage() {
  ...
  const top = card('API 小管家', '模型、Key、温度、top_p 都住这里');
  ...
  settings.apiEndpoints.forEach((api) => {
    const fixedApi = normalizeApiEndpoint(api);
    const isDefault = settings.defaultApiEndpointId === fixedApi.id;
    const paramsText = `温度：${fixedApi.temperature} · top_p：${fixedApi.topP} · 上限：${fixedApi.maxTokens}`;
    const item = card(
      fixedApi.name || '未命名 API',
      `${fixedApi.endpoint || '未填写地址'}\n模型：${fixedApi.model || '未选择'} · Key：${fixedApi.apiKey ? '已填写' : '未填写'}${isDefault ? ' · 默认' : ''}\n${paramsText}`
    );

    item.append(actionRow([
      actionBtn('star', '默认', () => setDefaultApi(fixedApi.id)),
      actionBtn('refresh', '拉模型', () => loadApiModels(fixedApi.id)),
      actionBtn('check', '测试', () => testApi(fixedApi.id)),
      actionBtn('edit', '编辑', () => openApiEditor(fixedApi)),
      actionBtn('delete', '删除', () => deleteApi(fixedApi.id))
    ]));

    if (Array.isArray(fixedApi.modelList) && fixedApi.modelList.length) {
      item.append(modelPicker({
        models: fixedApi.modelList,
        current: fixedApi.model,
        emptyText: '',
        onSelect: (model) => selectApiModel(fixedApi.id, model)
      }));
    }
...
function renderCloudPage() {
  ...
  form.append(actionRow([
    actionBtn('check', '测试连接', async () => {
      ...
      const saved = getCloud();

      saved.endpoint = next.endpoint;
      saved.apiKey = next.apiKey;
      saved.status = result.ok ? 'ok' : 'error';
      saved.lastTestAt = getNow();
      saved.updatedAt = getNow();

      setData(CLOUD_KEY, saved);
      showToast(result.ok ? '连上啦，云朵小仓库在线' : result.message || '没连上，检查地址或密钥哦');
      render('cloud');
    }),
  ...
  const sync = card('云端备份', '测试成功后，可以手动上传或恢复');
  sync.append(actionRow([
    actionBtn('upload', '上传到云端', async () => {
      const ok = await showConfirm('要把本机数据上传到云端吗？');
      if (!ok) return;
      ...
      try {
        ...
        showToast(error?.message || '上传失败了');
      }
    }),
    actionBtn('download', '从云端恢复', async () => {
      const ok = await showConfirm('会用云端数据覆盖本机数据，要继续吗？');
      if (!ok) return;

      try {
        showToast('正在从云端恢复...');
        await downloadSnapshotFromCloud({ overwrite: true, skipCloudConfig: true });
        showToast('恢复完成啦，刷新一下更稳');
        emitRefresh();
        render('cloud');
      } catch (error) {
        showToast(error?.message || '恢复失败了');
      }
    })
  ]));
  wrap.append(sync);
...
function renderWidgetsPage() {
  ...
  WIDGET_BG_LIST.forEach(([key, name]) => {
    bg.append(listAction('image', name, backgrounds[key]?.value ? '已换背景' : '还没换背景', [
      actionBtn('upload', '上传', () => uploadWidgetBg(key)),
      actionBtn('delete', '清除', () => clearWidgetBg(key))
    ], imagePreview(backgrounds[key]?.value || '', name, 'image')));
  });
  wrap.append(bg);
...
function renderIconsPage() {
  ...
  APP_LIST.forEach(([id, name]) => {
    ...
    list.append(listAction(hidden.has(id) ? 'settings' : 'star', custom.name || name, hidden.has(id) ? '已隐藏' : image ? '已换图' : '默认图标', [
...
function getSettings() {
  ...
  return {
    ...
    apiEndpoints: Array.isArray(saved.apiEndpoints) ? saved.apiEndpoints.map(normalizeApiEndpoint) : []
  };
}

function normalizeApiEndpoint(api = {}) {
  return {
    ...api,
    temperature: clampNumber(api.temperature ?? DEFAULT_API_PARAMS.temperature, 0, 2, DEFAULT_API_PARAMS.temperature),
    topP: clampNumber(api.topP ?? api.top_p ?? DEFAULT_API_PARAMS.topP, 0, 1, DEFAULT_API_PARAMS.topP),
    maxTokens: Math.round(clampNumber(api.maxTokens ?? api.max_tokens ?? DEFAULT_API_PARAMS.maxTokens, 1, 200000, DEFAULT_API_PARAMS.maxTokens))
  };
}
...
function openApiEditor(api) {
  const current = normalizeApiEndpoint(api || { id: generateId(), name: '', endpoint: '', apiKey: '', model: '', modelList: [] });
  ...
  const temperature = inputRow('Temperature 温度', String(current.temperature), '0 - 2，越高越活泼');
  const topP = inputRow('Top P', String(current.topP), '0 - 1，默认 1');
  const maxTokens = inputRow('Max Tokens 回复上限', String(current.maxTokens), '比如 1200');

  sheet.body.append(name.wrap, endpoint.wrap, apiKey.wrap, model.wrap, temperature.wrap, topP.wrap, maxTokens.wrap);

  if (Array.isArray(current.modelList) && current.modelList.length) {
    sheet.body.append(modelPicker({
      models: current.modelList,
      current: current.model,
      onSelect: (value) => {
        model.input.value = value;
        showToast(`先选这个：${value}`);
      }
    }));
  }
  ...
  sheet.actions.append(actionBtn('check', '保存', () => {
    const settings = getSettings();
    const next = normalizeApiEndpoint({
      id: current.id,
      ...
      topP: topP.input.value,
      maxTokens: maxTokens.input.value
    });
...
async function fetchTtsModels() {
  ...
  try {
    ...
    saveSettings(settings);
    showToast(`TTS 拉到 ${models.length} 个声音模型啦`);
    render('tts');
  } catch {
