...
// 依赖：core/ai-client.js, core/ui.js, core/util.js
...
injectStyle('popo-settings-ai-card', `
  ...
  .ai-card-hint.warn{color:#E8888C}
`);
...
export function renderAICard() {
  ...
  card.className = 'card';
  card.innerHTML = `
    <div class="card-title">AI 接口</div>
    ${configured ? '' : '<div class="ai-card-hint warn" id="ai-warn">还没配置 AI，聊天会用本地回复池</div>'}
    <div class="ai-card-field">
      <span class="ai-card-field-label">接口地址</span>
      <input class="input" id="ai-url" type="text" placeholder="https://api.openai.com/v1/chat/completions" value="${escapeAttr(cfg.url)}">
    </div>
    <div class="ai-card-field">
      <span class="ai-card-field-label">API Key</span>
    ...
    <div class="ai-card-field">
      <span class="ai-card-field-label">温度（越高越皮）</span>
    ...
    <div class="ai-card-actions">
      <button class="btn primary" id="ai-save" type="button">保存</button>
      <button class="btn" id="ai-test" type="button">测试一下</button>
    </div>
  `;
  ...
  card.querySelector('#ai-test').addEventListener('click', async () => {
    ...
    try {
      ...
      if (result.ok) {
        ...
        showToast('连不上，检查一下地址和 Key 嘛', 'error');
      }
    } catch (e) {
      showToast('连不上，检查一下地址和 Key 嘛', 'error');
    } finally {
