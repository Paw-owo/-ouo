...
function normalizeShopItem(item) {
  ...
  return {
    ...
    name: String(item.name || '未命名小物'),
    description: String(item.description || '这件小物还没写介绍，等你来补上。'),
...
async function createInventoryRow(record) {
  ...
  if (record.owner === 'character') {
    desc.textContent = `${record.characterName || 'TA'}收着这个小物：${record.item.effect || record.item.description}`;
  } else if (record.source === 'gift_from_ai') {
    desc.textContent = `这是TA送给你的：${record.item.effect || record.item.description}`;
  } else {
...
async function createEditorCard(item) {
  ...
  file.addEventListener('change', async () => {
    ...
    await setDB('blobs', `${ITEM_IMAGE_PREFIX}${item.id}`, {
      key: `${ITEM_IMAGE_PREFIX}${item.id}`,
      value,
      source: value,
      name: selected.name || '',
      type: selected.type || '',
  ...
  clearImage.addEventListener('click', async () => {
    await deleteDB('blobs', `${ITEM_IMAGE_PREFIX}${item.id}`);
    showToast('图片已经清掉啦');
    await renderShop();
  });
...
async function fillItemArt(element, item) {
  ...
  const record = await getDB('blobs', `${ITEM_IMAGE_PREFIX}${item.id}`);
  ...
  if (image) {
    ...
    img.addEventListener('error', () => {
      element.innerHTML = '';
      element.appendChild(createItemSvg(item.id));
    });
...
function getImageFromRecord(record) {
  ...
  const fields = ['value', 'source', 'data', 'image', 'iconImage', 'backgroundImage', 'imageBase64', 'url', 'src'];
...
export async function aiGiftToUser({ characterId, characterName = 'TA', itemId, message = '' } = {}) {
  ...
  await recordGiftMemory({
    ...
    role: 'assistant',
    source: '商店礼物',
    content: message || `我在小物商店里花 ¥${formatMoney(item.price)} 挑了「${item.name}」送给你。${item.effect || ''}`
  });

  window.dispatchEvent(new CustomEvent('shop-gift-created', {
    detail: { direction: 'ai_to_user', characterId, characterName, item }
  }));

  return { ok: true, item };
}

export async function userGiftToAI({ characterId, characterName = 'TA', itemId } = {}) {
  ...
  await writeGiftRecord({
    ...
    characterName,
    item,
    message: `用户把「${item.name}」送给了我。${item.effect || ''}`
  });
  ...
  await recordGiftMemory({
    ...
    role: 'user',
    source: '商店礼物',
    content: `用户把「${item.name}」送给了我。${item.effect || ''}`
  });
...
async function writeGiftRecord({ direction, characterId, characterName, item, message }) {
  const record = {
    id: `gift_${generateId()}`,
    direction,
...
// 依赖：../core/storage.js 的 getData/setData/generateId/getNow/getAllDB/setDB/getDB/deleteDB；../core/ui.js 的 showToast/showConfirm/createIcon；./wallet.js 的 getBalance/deductBalance/deductAiBalance；可选动态依赖 ./chat.js 的 recordExternalInteraction
