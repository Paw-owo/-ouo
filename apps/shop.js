...
export async function aiGiftToUser({ characterId, characterName = 'TA', itemId, message = '' } = {}) {
  ...
  await recordGiftMemory({
    ...
    role: 'assistant',
    source: '商店礼物',
    content: message || `我在小物商店里挑了「${item.name}」送给你。${item.effect || ''}`
  });
...
// 依赖：../core/storage.js 的 getData/setData/generateId/getNow/getAllDB/setDB/getDB/deleteDB；../core/ui.js 的 showToast/showConfirm/createIcon；./wallet.js 的 getBalance/deductBalance；可选动态依赖 ./chat.js 的 recordExternalInteraction
