...
function normalizeTransaction(item) {
  return {
    ...
    category: item.category || (item.type === 'expense' ? 'expense' : 'income'),
...
function addTransaction(wallet, amount, description, type, extra = {}) {
  const transaction = {
    ...
    timestamp: getNow(),
    type,
    category: extra.category || (type === 'income' ? 'income' : 'expense'),
...
export function addBalance(amount, description = '充值') {
  ...
  const nextWallet = addTransaction(wallet, value, description, 'income', { category: 'income' });
...
export function deductBalance(amount, description = '消费') {
  ...
  const nextWallet = addTransaction(wallet, value, description, 'expense', { category: 'expense' });
  nextWallet.balance = wallet.balance;
...
export function addAiBalance(characterId, amount, description = '收入', extra = {}) {
  ...
  const nextWallet = addTransaction(wallet, value, description, 'income', extra);
  nextWallet.balance = wallet.balance;
  wallets[characterId] = nextWallet;
...
export function deductAiBalance(characterId, amount, description = '支出', extra = {}) {
  ...
  const nextWallet = addTransaction(wallet, value, description, 'expense', extra);
  nextWallet.balance = wallet.balance;
  wallets[characterId] = nextWallet;
...
export async function transferToAI({ characterId, characterName = 'TA', amount, note = '' } = {}) {
  ...
  const nextWallet = addTransaction(
    wallet,
    value,
    `转给${characterName}${note ? `：${note}` : ''}`,
    'expense',
    { category: 'transfer', characterId, characterName }
  );
  nextWallet.balance = wallet.balance;
  saveWallet(nextWallet);

  addAiBalance(characterId, value, `收到用户转账${note ? `：${note}` : ''}`, {
    category: 'transfer',
    characterId,
    characterName
  });
  ...
  await recordWalletMemory({
    ...
    role: 'user',
    source: '钱包转账',
    content: `用户转给我 ¥${formatMoney(value)}${note ? `，备注是：${note}` : ''}。我会记得这份小小的照顾。`
  });
...
export async function aiTransferToUser({ characterId, characterName = 'TA', amount, note = '' } = {}) {
  ...
  const paid = deductAiBalance(characterId, value, `转给用户${note ? `：${note}` : ''}`, {
    category: 'transfer',
    characterId,
    characterName
  });
  ...
  await recordWalletMemory({
    ...
    role: 'assistant',
    source: '钱包转账',
    content: `我转给用户 ¥${formatMoney(value)}${note ? `，备注是：${note}` : ''}。`
  });

  window.dispatchEvent(new CustomEvent('wallet-transfer-created', {
    detail: { direction: 'ai_to_user', characterId, characterName, amount: value, note }
  }));
