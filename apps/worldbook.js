...
function injectStyles() {
  ...
  style.textContent = `
    ...
    .wb-soft-layer{position:absolute;inset:0;z-index:0;pointer-events:none;background:color-mix(in srgb,var(--bg-primary) 78%,transparent)}
    .wb-nav{position:fixed;top:0;left:0;right:0;z-index:100;height:calc(56px + env(safe-area-inset-top));display:flex;align-items:center;gap:var(--spacing-sm);padding:env(safe-area-inset-top) 20px 0;background:var(--surface-glass);backdrop-filter:blur(18px);-webkit-backdrop-filter:blur(18px)}
    ...
    .wb-hero{padding:18px;margin-bottom:var(--spacing-md);border-radius:28px;background:var(--bg-card);box-shadow:var(--shadow-md)}
    ...
    .wb-tab-bar{display:flex;gap:var(--spacing-xs);padding:var(--spacing-xs);margin-bottom:var(--spacing-md);border-radius:var(--radius-md);background:var(--surface-muted)}
    ...
    .wb-card{background:var(--bg-card);border-radius:24px;padding:var(--spacing-md);box-shadow:var(--shadow-sm);transition:var(--motion);cursor:pointer;overflow:hidden}
...
async function loadVisualCache(entries) {
  await Promise.all(entries.map(async (entry) => {
    if (!coverCache.has(entry.id)) {
      ...
      if (cover?.value) coverCache.set(entry.id, cover.value);
    }

    if (!iconCache.has(entry.id)) {
      const icon = await getDB('blobs', getIconKey(entry.id)).catch(() => null);
      if (icon?.value) iconCache.set(entry.id, icon.value);
    }
  }));
}
...
function openEntryImageSheet(entry) {
  ...
  coverSection.querySelector('.wb-custom-actions').append(
    createUploadButton('上传封面', getCoverKey(entry.id), async (value) => {
      ...
      setVisualMeta(entry.id, { name: entry.title, opacity: 1 });
      await renderList();
    }),
  ...
  sheet.append(title, coverSection, iconSection);
  showBottomSheet(sheet);
}
